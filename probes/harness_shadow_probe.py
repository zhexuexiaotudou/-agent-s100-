...
def existing_gate_presence(root: Path, production_context_root: Path) -> dict[str, Any]:
    missing = []
    details: dict[str, Any] = {}
    try:
        details.update(critical_asset_map(root, production_context_root))
    except FileNotFoundError as exc:
        missing.append(str(exc))
    gate_checks = {
        "openclaw_nas_control_gate": [
            root / "scripts" / "probes" / "ai_nas_openclaw_nas_control_gate_probe.py",
            root / "existing_gate_evidence" / "openclaw_nas_control_gate_latest.json",
            root / "tmp" / "harness_existing_gates" / "openclaw" / "openclaw_nas_control_gate_latest.json",
        ],
        "edge_cloud_router_gate": [
            root / "scripts" / "probes" / "ai_nas_edge_cloud_router_probe.py",
            root / "existing_gate_evidence" / "edge_cloud_router.json",
        ],
        "qwen_acceptance_packet": [
            root / "scripts" / "probes" / "qwen25_ai_nas_acceptance_packet.py",
            root / "existing_gate_evidence" / "qwen25_ai_nas_acceptance_latest.json",
            root / "tmp" / "harness_existing_gates" / "qwen_acceptance" / "qwen25_ai_nas_acceptance_latest.json",
        ],
    }
    resolved_details = {key: str(value) for key, value in details.items()}
    for name, candidates in gate_checks.items():
        found = next((path for path in candidates if path.exists()), None)
        if found:
            resolved_details[name] = str(found)
        else:
            missing.append(name)
    return {
        "status": "pass" if not missing else "fail",
        "missing": missing,
        "required_existing_asset_missing": bool(missing),
        "service_context_missing": any("service" in item or "qwen_gateway" in item or "dispatcher" in item for item in missing),
...
def main() -> int:
    ...
    parser.add_argument("--production-context-root", type=Path, default=None)
    args = parser.parse_args()
    repo_root_path = find_repo_root(args.repo_root or ROOT)
    production_context_root = find_production_context_root(repo_root_path, args.production_context_root)
    ...
    registry = load_json_yaml(repo_root_path / "config" / "workspace_registry.yaml")
    policy = load_json_yaml(repo_root_path / "config" / "workspace_tool_policy.yaml")
    all_tool_ids = sorted((policy.get("tool_catalog") or {}).keys())
    writer = RuntimeTraceWriter(trace_db)
    tool_filter = ToolExposureFilter(policy_path=repo_root_path / "config" / "workspace_tool_policy.yaml", trace_writer=writer)
    memory = MemoryStore(memory_db)
    seed_memory(memory)

    gate_presence = existing_gate_presence(repo_root_path, production_context_root)
    scenario_results = []
    for scenario in SCENARIOS:
        ...
        if selected == "web_cloud_research":
            redaction = redact_cloud_payload(scenario["user_request"])
            redaction_payload = redaction.to_dict()
            cloud_payload = {
                "allowed": cloud_allowed,
                "egress_payload_hash": redaction_payload["egress_payload_hash"],
                "egress_preview": redaction_payload["redacted_preview"],
                "redaction_applied": redaction_payload["redaction_applied"],
                "redaction_summary": redaction_payload["redaction_summary"],
                "contains_private_terms_after_redaction": redaction_payload["redaction_summary"]["leak_count"] > 0,
            }
        writer.add_step(
            run_id,
            "context_built",
        ...
        scenario_results.append(
            {
                ...
                "required_existing_asset_missing": bool(gate_presence.get("required_existing_asset_missing")),
                "service_context_missing": bool(gate_presence.get("service_context_missing")),
            }
        )
    ...
    for item in scenario_results:
        ...
        if not item["existing_gates_still_pass"]:
            failures.append(f"{item['scenario_id']}:existing_gates_not_pass")
        if item.get("required_existing_asset_missing"):
            failures.append(f"{item['scenario_id']}:required_existing_asset_missing")
        if item.get("service_context_missing"):
            failures.append(f"{item['scenario_id']}:service_context_missing")
