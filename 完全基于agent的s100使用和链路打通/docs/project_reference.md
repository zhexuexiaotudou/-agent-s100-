...
Verified top-window round3 NAS report:
...
Verified top-window round3 S100P local-cache report:
...
Verified top-window round3 fields copied from `resplit_hbm_artifact_inventory_probe.json`:
...
Verified top-window round3 report:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260609-153118/resplit_batch_telemetry_probe.json
```

Verified top-window round3 fields copied from `resplit_batch_telemetry_probe.json`:

```text
verdict: ok_dream7b_bpu_resplit_batch_telemetry_probe
batch_count: 16
expected_segment_plan: resplit-topwindow-adjacent
expected_segment_event_count: 288
max_bpu_loading: 100.0
avg_bpu_loading: 9.628
forward_metrics.segment_plan: resplit-topwindow-adjacent
forward_metrics.segment_event_count: 288
forward_metrics.expected_segment_event_count: 288
forward_metrics.segment_sources: ['base', 'fine', 'resplit', 'topwindow']
forward_metrics.load_ms: 23166.108
forward_metrics.run_ms: 2446.735
forward_metrics.load_to_run_ratio: 9.468172
forward_metrics.amortized_load_ms_per_forward: 1447.882
forward_metrics.amortized_run_ms_per_forward: 152.921
forward_metrics.topwindow_hbm_dir: /home/sunrise/.cache/openclaw/dream7b-hbm/resplit-topwindow-seq16
errors: []
```
...
Verified top-window round3 window-cost report:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260609-153244/resplit_window_cost_probe.json
```

Verified top-window round3 window-cost fields copied from `resplit_window_cost_probe.json`:
...
amortized_load_ms_per_forward: 1447.88175
amortized_run_ms_per_forward: 152.920938
top_load_window.resident_segments: ['seg02_04', 'seg04_07']
top_load_window.load_ms: 3564.522
top_load_to_run_ratio_window.resident_segments: ['seg00_01', 'seg01_02']
top_load_to_run_ratio_window.load_to_run_ratio: 18.08781
next_optimization_target: reduce packed HBM load cost for top ranked resplit windows before expecting sustained 128TOPS-level average utilization
errors: []
```
...
Source file: `scripts/probes/s100_official_qwen_fullflow_probe.sh`
...
S100_OFFICIAL_QWEN_FULLFLOW_VENV_DIR
...
S100_OFFICIAL_QWEN_FULLFLOW_RUNTIME_TIMEOUT_SECONDS
S100_OFFICIAL_QWEN_FULLFLOW_MONITOR_DELAY_MS
S100_OFFICIAL_QWEN_FULLFLOW_MONITOR_SAMPLE_COUNT
S100_OFFICIAL_QWEN_FULLFLOW_RUN_RUNTIME
```
...
source_model_dir
source_model_exists
supported_models_from_sdk_wheel
model_registered
build_host_compatible
build_status
build_pass
compiled_hbm_path
compiled_hbm_size_bytes
...
```text
This probe is the official Qwen source-model to HBM to isolated runtime baseline.
It must not overwrite the vendor qwen_multichat_config.json and does not make Qwen
the default Dream 7B target.
```

Current WSL build artifact report:

```text
/mnt/nas/openclaw/reports/models/qwen_fullflow_wsl_build_report_20260609-081247/official_qwen_fullflow_probe.json
```

Current WSL build artifact fields copied from `official_qwen_fullflow_probe.json`:

```text
verdict: failed_s100_official_qwen_fullflow_probe
model_name: qwen2_5-1_5b
march: nash-m
cache_len: 1024
chunk_size: 256
...
errors: ['oellm_build failed with returncode 1']
failure interpretation: HBM was emitted, then host-side HBM load/link verification failed because WSL has no ION/common-buffer device.
```

Current S100P runtime report:

```text
/mnt/nas/openclaw/reports/models/s100_official_qwen_fullflow_20260609-172710/official_qwen_fullflow_probe.json
```

Current S100P runtime fields copied from `official_qwen_fullflow_probe.json`:
...
errors: ['build host is not compatible: x86_64 with AVX is required for HBDK compiler import']
```

### `dream7b-oellm-fullflow-feasibility-probe`

Source file: `scripts/probes/dream7b_oellm_fullflow_feasibility_probe.sh`
...
DREAM7B_OELLM_FULLFLOW_VENV_DIR
...
output_dir = /mnt/nas/openclaw/models/dream7b-oellm-fullflow
compile_script = scripts/probes/compile_dream_with_deepseek_skeleton.sh
march = nash-m
chunk_size = 256
cache_len = 1024
w_bits = 8
...
compile_dream_with_deepseek_skeleton.stdout.txt
compile_dream_with_deepseek_skeleton.stderr.txt
...
```text
This probe is a zero-promotion Dream migration gate. If the direct OELLM path is
blocked, the failure package is the handoff artifact for teacher/vendor help, and
the existing segmented Dream HBM route remains the main path.
```

Current S100P report:
...
Current S100P fields copied from `dream7b_oellm_fullflow_feasibility_probe.json`:

```text
verdict: failed_dream7b_oellm_fullflow_feasibility_probe
dream_registered_in_official_sdk: false
supported_models_from_sdk: ['deepseek-qwen-1_5b', 'deepseek-qwen-7b', 'qwen2_5-1_5b', 'qwen2_5-7b', 'internvl2-2b', 'internvl2_5-2b', 'internvl2-1b', 'internvl2_5-1b', 'internlm2-1_8b', 'qwen2_5-omni-3b']
dream_config_summary.model_type: Dream
dream_config_summary.num_hidden_layers: 28
dream_config_summary.hidden_size: 3584
...
- Updated `dream7b_segmented_hbm_python_forward.py`, `dream7b-bpu-utilization-gap-probe`, and `dream7b-bpu-deployment-acceptance-probe` for round3 top-window evidence; verified `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260609-153933/utilization_gap_probe.json` with `diagnosis: hbm_reload_dominated`, `resplit_batch_telemetry.load_to_run_ratio: 9.468`, `resplit_batch_telemetry.avg_bpu_loading: 9.628`, `resplit_window_cost.window_count: 9`, and `resplit_window_cost.top_load_window.resident_segments: ['seg02_04', 'seg04_07']`, then verified `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260609-154013/deployment_acceptance_probe.json` with `check_count: 30`, `passed_check_count: 30`, and `utilization_gap.ok: True`.
...
## 2026-06-09 Qwen 512/128 and Dream Migration Acceptance

- Current official Qwen runnable baseline: Qwen2.5-1.5B with `cache_len: 512` and `chunk_size: 128`. HBM path is `/mnt/nas/openclaw/models/s100-official-qwen-fullflow/cache_len_512_chunk_128/qwen2_5-1_5b_chunk_128_cache_512_q8.hbm`. Runtime report `/mnt/nas/openclaw/reports/models/s100_official_qwen_fullflow_20260609-210514/official_qwen_fullflow_probe.json` records `runtime_completed: true`, `runtime_returncode: 0`, `memory_alloc_failure_observed: false`, `nonzero_bpu_loading_sample_count: 6`, `max_bpu_loading: 98.0`, `avg_bpu_loading: 2.222`, and `Performance prefill: 1024.00tokens/s    decode: 25.57tokens/s`. Diagnosis report `/mnt/nas/openclaw/reports/models/s100_qwen15_common_buffer_diagnosis_20260609-211000/s100_qwen15_common_buffer_diagnosis_20260609.md` keeps 1024/256 as a high-context failure case for common-buffer/BPU allocation.
- Current Dream official OELLM migration result: `/mnt/nas/openclaw/reports/models/dream7b_oellm_fullflow_feasibility_20260609-223754/dream7b_oellm_fullflow_feasibility_probe.json` records `host_machine: x86_64`, `host_has_avx: true`, `build_host_compatible: true`, `dream_registered_in_official_sdk: false`, `supported_models_from_sdk: ['deepseek-qwen-1_5b', 'deepseek-qwen-7b', 'qwen2_5-1_5b', 'qwen2_5-7b', 'internvl2-2b', 'internvl2_5-2b', 'internvl2-1b', 'internvl2_5-1b', 'internlm2-1_8b', 'qwen2_5-omni-3b']`, `compile_status: blocked_registry_missing`, `failure_stage: registry_missing`, `compiled_hbm_count: 0`, `direct_oellm_migration_supported: false`, `missing_adapter_evidence.registry_missing: true`, `missing_adapter_evidence.required_adapter: official leap_llm model_factory registration and Dream/DreamModel adapter`, and `unable_to_attempt_direct_official_compile_reason`. Dream source/config is `/mnt/f/Project/Digua/tmp/dream_hf/config.json`; summary fields are `model_type: Dream`, `architectures: ['DreamModel']`, `hidden_size: 3584`, `num_hidden_layers: 28`, `num_attention_heads: 28`, `num_key_value_heads: 4`, `vocab_size: 152064`, `mask_token_id: 151666`, `torch_dtype: bfloat16`, and `use_cache: true`.
- Current Dream segmented HBM refresh: artifact inventory reports `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_hbm_artifact_inventory_20260609-224012/resplit_hbm_artifact_inventory_probe.json` and `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_hbm_artifact_inventory_20260609-224035/resplit_hbm_artifact_inventory_probe.json` verify `expected_hbm_count: 8`, `existing_hbm_count: 8`, `manifest_verified_count: 8`, and `total_hbm_size_bytes: 2512771264`. Batch telemetry `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260609-224100/resplit_batch_telemetry_probe.json` records `avg_bpu_loading: 8.97` and `forward_metrics.load_to_run_ratio: 9.678265`; window-cost `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260609-224142/resplit_window_cost_probe.json` records `window_count: 9` and `load_to_run_ratio: 9.678265`; utilization-gap `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260609-224156/utilization_gap_probe.json` records `diagnosis: hbm_reload_dominated`; deployment acceptance `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260609-224157/deployment_acceptance_probe.json` records `check_count: 30` and `passed_check_count: 30`. The latest `9.678265` does not beat the current best `9.468172`, though it remains better than historical `9.694618`; keep the status as `hbm_reload_dominated`.

## 2026-06-10 Official DeepSeek 7B Fallback Decision

- Probe: `scripts/probes/s100_official_deepseek7b_baseline_probe.sh`
- Decision report: `docs/deepseek7b_s100p_fallback_decision_2026-06-10.md`
- Latest report: `/mnt/nas/openclaw/reports/models/s100_official_deepseek7b_baseline_20260610-023455/deepseek7b_baseline_probe.json`
- HBM: `/mnt/nas/openclaw/models/s100-official-deepseek-r1-distill-qwen-7b/DeepSeek_R1_Distill_Qwen_7B_1024.hbm`
- Key fields: `hbm_exists: True`, `hbm_size_bytes: 7928846896`, `runtime_status: failed`, `runtime_returncode: 1`, `runtime_completed: False`, `hbm_load_success_observed: False`, `init_model_success_observed: False`, `memory_alloc_failure_observed: True`, `bpu_alloc_request_bytes: 7928846896`, `decision: official_7b_runtime_blocked_common_buffer`.
- Comparison decision: Qwen2.5-1.5B `512/128` remains the verified official runnable fallback. Dream 7B official OELLM remains blocked at `registry_missing`; Dream segmented HBM remains `hbm_reload_dominated`. DeepSeek-R1-Distill-Qwen-7B has a complete official HBM but is blocked by common-buffer/BPU allocation on the current S100P memory layout, so it is not yet a deployable 7B fallback.

## 2026-06-10 Dream Reload Optimization And Vendor Support Closure

- Vendor package: `docs/dream7b_vendor_support_package_2026-06-10.md`
- Reload optimization probe: `scripts/probes/dream7b_bpu_reload_optimization_probe.sh`
- Latest telemetry: `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260610-030740/resplit_batch_telemetry_probe.json`
- Latest window cost: `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260610-030820/resplit_window_cost_probe.json`
- Latest selected-pair telemetry: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_telemetry_20260610-031613/selected_pair_telemetry_probe.json`
- Latest utilization gap: `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260610-031818/utilization_gap_probe.json`
- Latest acceptance: `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-031818/deployment_acceptance_probe.json`
- Reload decision: `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-031828/reload_optimization_probe.json`
- Key fields: `avg_bpu_loading: 8.811`, `max_bpu_loading: 100.0`, `forward_metrics.load_to_run_ratio: 9.524202`, `forward_metrics.wall_ms: 26007.248`, `selected_pair: [1, 8]`, `selected_pair_covers_all_segments: True`, `selected_pair.wall_ms: 22982.452`, `selected_pair.avg_bpu_loading: 8.984`, `selected_pair_wall_delta_ratio_vs_resplit: 0.116306`, `selected_pair_avg_bpu_delta_vs_resplit: 0.173`, `diagnosis: hbm_reload_dominated`, `check_count: 30`, `passed_check_count: 30`, `final_decision: utilization_progress_candidate`, `substantial_improvement_observed: True`, `best_load_to_run_ratio: 9.468172`, and `ratio_delta_vs_best: 0.05603`.
- Strategy matrix: `file_prefetch_or_local_hbm_cache: bounded_prefetch_only`, `persistent_pair_cache: blocked_by_residency_boundary`, `persistent_segment_cache: blocked_by_residency_boundary`, `persistent_triplet_topology: topology_stable_but_forward_blocked`, `selected_pair_cross_job_cache: batch16_wall_candidate`, `window_scheduling_resplit_topwindow: not_better_than_current_best`, and `bpu_core_scheduling: core0_only`.
- Decision: do not replace the default service yet, but selected-pair resident execution is a guarded utilization-progress candidate because batch16 wall time improves by `0.116306` and average BPU loading improves by `0.173` against the latest top-window run. The next internal work is sustained selected-pair service telemetry plus rollback-gated promotion testing.

## 2026-06-10 Sustained Selected-Pair Service Candidate

- Sustained service telemetry: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_candidate_service_telemetry_20260610-032523/systemd_telemetry_probe.json`
- Utilization gap: `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260610-032709/utilization_gap_probe.json`
- Acceptance: `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-032709/deployment_acceptance_probe.json`
- Reload decision: `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-032926/reload_optimization_probe.json`
- Key fields: `processed_request_count: 48`, `batch_counts: [16, 16, 16]`, `amortized_wall_ms_per_processed_request: 1445.673`, `avg_bpu_loading: 8.371`, `max_bpu_loading: 98.0`, `load_to_run_ratio: 9.859028`, `wall_ms_delta_ratio_vs_default_systemd: 0.130546`, `candidate_avg_bpu_loading_not_worse_than_default_systemd: False`, `diagnosis: hbm_reload_dominated`, `check_count: 30`, `passed_check_count: 30`, `wall_delta_ratio_vs_resplit_per_request: 0.110603`, `avg_bpu_delta_vs_resplit: -0.44`, and `avg_bpu_delta_vs_selected_pair_single: -0.613`.
- Decision: `final_decision: guarded_sustained_wall_time_candidate`, `sustained_service_decision: guarded_sustained_wall_time_candidate`, and `default_service_replacement_decision: do_not_replace_default_service_yet`. This advances selected-pair service residency to a rollback-gated wall-time candidate only; average BPU loading and load/run remain insufficient for a default-service promotion or solved 128TOPS utilization claim.

## 2026-06-10 Selected-Pair Service Promotion Gate

- Probe: `scripts/probes/dream7b_bpu_selected_pair_service_promotion_gate_probe.sh`
- Report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_service_promotion_gate_20260610-141720/selected_pair_service_promotion_gate_probe.json`
- Key fields: `verdict: ok_dream7b_bpu_selected_pair_service_promotion_gate_probe`, `promotion_allowed: False`, `promotion_decision: block_default_service_replacement`, `default_service_replaced: False`, `candidate_service_isolated_from_default: True`, `processed_request_count: 48`, `candidate_load_to_run_ratio: 9.859028`, `wall_delta_ratio_vs_default_systemd: 0.130546`, `wall_delta_ratio_vs_resplit_per_request: 0.110603`, `avg_bpu_delta_vs_default_systemd: -1.245`, `avg_bpu_delta_vs_resplit: -0.44`, and `utilization_diagnosis: hbm_reload_dominated`.
- Promotion blockers: `promotion_average_bpu_not_worse_vs_default`, `promotion_average_bpu_improved_vs_resplit`, `promotion_load_to_run_not_worse_than_best`, `promotion_not_hbm_reload_dominated`, and `reload_gate_allows_default_replacement`.
- Rollback commands are embedded in the report: `sudo systemctl stop dream7b-bpu-selected-pair-candidate.service`, `sudo systemctl disable dream7b-bpu-selected-pair-candidate.service`, and `sudo systemctl restart dream7b-bpu-batch-queue.service`.
- Decision: keep `dream7b-bpu-batch-queue.service` as the default service. Selected-pair remains candidate-only until it improves average BPU loading or load/run under sustained telemetry.

## 2026-06-10 Promotion Blocker Diagnosis

- Probe: `scripts/probes/dream7b_bpu_promotion_blocker_diagnosis_probe.sh`
- Report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_promotion_blocker_diagnosis_20260610-145739/promotion_blocker_diagnosis_probe.json`
- Key fields: `verdict: ok_dream7b_bpu_promotion_blocker_diagnosis_probe`, `promotion_decision: keep_candidate_only_until_blockers_clear`, `candidate_load_to_run_ratio: 9.859028`, `load_to_run_delta_vs_best: 0.390856`, `load_to_run_delta_vs_resplit: 0.334826`, `service_avg_bpu_loading: 8.371`, `avg_bpu_delta_vs_resplit: -0.44`, `wall_delta_ratio_vs_resplit_per_request: 0.110603`, `top_load_to_run_ratio_window.resident_segments: ['seg00_01', 'seg01_02']`, `top_load_to_run_ratio_window.load_to_run_ratio: 18.585073`, `top_load_window.resident_segments: ['seg02_04', 'seg04_07']`, `top_load_window.load_ms: 3562.79`, `max_resident_segment_count_observed: 3`, `successful_seeded_quad_count: 0`, and `recommended_resplit_segment_indexes: [0, 9, 4, 6]`.
- Next candidates: `prefix_micro_window_reload_reduction`, `seg02_04_seg04_07_window_cost_reduction`, `resident_capacity_boundary_experiment`, and `core0_only_scheduling_control`.
- Decision: the selected-pair service remains candidate-only. The next large experiment should reduce prefix/top-load window reload cost, then rerun sustained service telemetry and the promotion gate.

## 2026-06-10 Final Optimization Acceptance

- Probe: `scripts/probes/dream7b_final_optimization_acceptance_probe.sh`
- Report: `/mnt/nas/openclaw/reports/models/dream7b_final_optimization_acceptance_20260610-161744/final_optimization_acceptance_probe.json`
- Key fields: `verdict: ok_dream7b_final_optimization_acceptance_probe`, `final_goal_satisfied: True`, `final_decision: dream7b_bpu_optimized_candidate_complete_candidate_only`, `deployment_route_closed: True`, `substantial_improvement_observed: True`, `utilization_statement_compliant: True`, `promotion_gate_closed: True`, `dream_oellm_registry_missing: True`, `qwen_fallback_verified: True`, `deepseek7b_blocked_common_buffer: True`, `candidate_service_only: True`, `default_service_replaced: False`, `selected_pair_wall_delta_ratio_vs_resplit: 0.116306`, `sustained_wall_delta_ratio_vs_resplit_per_request: 0.110603`, `sustained_wall_delta_ratio_vs_default_systemd: 0.130546`, and `sustained_request_count: 48`.
- Final decision: Dream 7B now has a reproducible, accepted, rollback-gated BPU optimization candidate with measured wall-time improvement. The accepted endpoint is candidate-only, not default-service replacement, because average BPU and load/run still block promotion.

## 2026-06-10 Normal-Use Candidate Acceptance

- Probe: `scripts/probes/dream7b_bpu_normal_use_acceptance_probe.sh`
- Report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_normal_use_acceptance_20260610-170005/normal_use_acceptance_probe.json`
- Key fields: `verdict: ok_dream7b_bpu_normal_use_acceptance_probe`, `normal_use_ready: True`, `unmet_requirements: []`, `stable_sustained_service: True`, `preferred_96_request_sustained_observed: True`, `deployment_acceptance_clean: True`, `performance_floor_met: True`, `reload_relief_observed: True`, `deployable_rollback_safe: True`, and `utilization_claim_compliant: True`.
- Sustained service evidence: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_candidate_service_telemetry_20260610-165645/systemd_telemetry_probe.json` records `processed_request_count: 48`, `batch_counts: [16, 16, 16]`, `failed_job_count: 0`, `amortized_wall_ms_per_processed_request: 1448.877`, `avg_bpu_loading: 8.578`, `max_bpu_loading: 96.0`, `wall_delta_ratio_vs_resplit_per_request: 0.102517`, `wall_delta_ratio_vs_default_systemd: 0.128619`, and `load_to_run_ratio: 9.863791`. The preferred 96-request stability evidence remains `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_candidate_service_telemetry_20260610-163241/systemd_telemetry_probe.json`.
- Reload experiment evidence: `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260610-165429/resplit_batch_telemetry_probe.json` records `expected_segment_plan: resplit-topwindow-adjacent`, `avg_bpu_loading: 8.783`, `max_bpu_loading: 98.0`, and `forward_metrics.load_to_run_ratio: 9.443895`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260610-165513/resplit_window_cost_probe.json` records `load_to_run_ratio: 9.443895`, improving over previous `9.774037`, with `top_ratio_improved: True`, `top_load_improved: True`, and `overall_load_to_run_improved: True`.
- Decision: Dream 7B now meets the normal-use candidate threshold as an isolated selected-pair service. Keep `dream7b-bpu-batch-queue.service` as the default because sustained `avg_bpu_loading` and `load_to_run_ratio` still miss promotion targets; keep the status wording `hbm_reload_dominated` unless a later sustained run clears those two metrics.

## 2026-06-10 Default-Deployable Final Acceptance

- Probe: `scripts/probes/dream7b_bpu_default_deployable_acceptance_probe.sh`
- Report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-173758/default_deployable_acceptance_probe.json`
- Key fields: `verdict: ok_dream7b_bpu_default_deployable_acceptance_probe`, `default_deployable_ready: False`, `default_deployable_status: blocked_candidate_only`, `processed_request_count: 192`, `failed_job_count: 0`, `preferred_sustained_observed: True`, `deployment_acceptance_clean: True`, `promotion_allowed: False`, `performance_pass_count: 1`, `min_performance_pass_count: 3`, `amortized_wall_ms_per_processed_request: 1450.4`, `avg_bpu_loading: 8.476`, and `load_to_run_ratio: 9.873488`.
- Service blockers: `service_performance_pass_count_below_3`, `service_avg_bpu_below_default_deploy_threshold`, `service_load_to_run_above_default_deploy_threshold`, `promotion_average_bpu_not_worse_vs_default`, `promotion_average_bpu_improved_vs_resplit`, `promotion_load_to_run_not_worse_than_best`, `promotion_not_hbm_reload_dominated`, and `reload_gate_allows_default_replacement`.
- Promotion and reload evidence: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_service_promotion_gate_20260610-173049/selected_pair_service_promotion_gate_probe.json` keeps `promotion_decision: block_default_service_replacement`, and `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-173049/reload_optimization_probe.json` keeps `default_service_replacement_decision: do_not_replace_default_service_yet` with `utilization_diagnosis: hbm_reload_dominated`.
- Cross-job prototype evidence: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_reuse_20260610-172408/selected_pair_cross_job_reuse_probe.json` records `processed_forward_count: 128`, `load_to_run_ratio: 8.895442`, `amortized_wall_ms_per_forward: 1448.768`, `load_ms_delta_ratio_vs_service: 0.10225`, `cross_job_wall_time_improved: True`, and `performance_pass_count: 2`; this proves HBM reload can be reduced by cross-job reuse, but it is not a deployed service and cannot promote the default service.
- Decision: the final default-deployable gate is closed as `blocked_candidate_only`. The selected-pair candidate remains usable for normal workloads, but the default Dream 7B service must stay unchanged until three performance metrics pass and the promotion gate reports `promotion_allowed: True`.

## 2026-06-10 Cross-Job Queue Runner Candidate

- Runner: `scripts/dream7b_bpu_selected_pair_cross_job_queue_runner.py`
- Forward-path interface update: `scripts/probes/dream7b_bpu_selected_pair_forward_path_probe.sh` now accepts `DREAM7B_BPU_SELECTED_PAIR_TOKENS_BATCHES_BY_JOB_JSON` and permits `DREAM7B_BPU_SELECTED_PAIR_JOB_COUNT` up to 12 for selected-only cross-job queue runs.
- 6x16 report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_runner_20260610-175302/cross_job_queue_summary.json`
- 6x16 key fields: `verdict: ok_dream7b_bpu_selected_pair_cross_job_queue_runner`, `processed_job_count: 6`, `processed_request_count: 96`, `failed_job_count: 0`, `load_to_run_ratio: 8.890613`, `amortized_wall_ms_per_processed_request: 1438.937`, `amortized_total_load_ms_per_processed_request: 1309.349`, `amortized_run_ms_per_processed_request: 147.273`, and `selected_pair_covers_all_segments: True`.
- 12x16 report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_runner_20260610-175739/cross_job_queue_summary.json`
- 12x16 key fields: `verdict: ok_dream7b_bpu_selected_pair_cross_job_queue_runner`, `processed_job_count: 12`, `processed_request_count: 192`, `failed_job_count: 0`, `load_to_run_ratio: 8.904793`, `amortized_wall_ms_per_processed_request: 1459.145`, `amortized_total_load_ms_per_processed_request: 1311.437`, `amortized_run_ms_per_processed_request: 147.273`, and `selected_pair_covers_all_segments: True`.
- Refreshed gates: `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-180307/deployment_acceptance_probe.json` records `verdict: ok_dream7b_bpu_deployment_acceptance_probe`, `check_count: 30`, `passed_check_count: 30`, and `errors: []`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_service_promotion_gate_20260610-180307/selected_pair_service_promotion_gate_probe.json` records `verdict: ok_dream7b_bpu_selected_pair_service_promotion_gate_probe`, `promotion_allowed: False`, and `promotion_decision: block_default_service_replacement`.
- Refreshed final gate: `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-180307/default_deployable_acceptance_probe.json` records `verdict: ok_dream7b_bpu_default_deployable_acceptance_probe`, `default_deployable_ready: False`, `default_deployable_status: blocked_candidate_only`, `cross_job_queue_candidate.preferred_sustained_observed: True`, `cross_job_queue_candidate.performance_pass_count: 1`, `cross_job_queue_candidate.load_to_run_ratio: 8.904793`, and `cross_job_queue_candidate.amortized_wall_ms_per_processed_request: 1459.145`.
- Decision: the queue runner proves selected-pair cross-job reuse can be expressed as a real queue-runner candidate and can reduce `load_to_run_ratio` below 9.0. It still cannot promote the default service because the 192-request wall time regresses and the candidate only passes one default-deploy performance check in the refreshed final gate.

## 2026-06-10 Cross-Job Candidate Service Promotion Gate

- Service installer: `scripts/install_dream7b_bpu_selected_pair_cross_job_candidate_service.sh`
- Service loop: `scripts/dream7b_bpu_selected_pair_cross_job_queue_service.py`
- Supporting probes: `scripts/probes/dream7b_bpu_selected_pair_cross_job_queue_telemetry_probe.sh` and `scripts/probes/dream7b_bpu_selected_pair_cross_job_service_promotion_gate_probe.sh`
- Service name: `dream7b-bpu-selected-pair-cross-job-candidate.service`
- Service telemetry report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_candidate_service_telemetry_20260610-182409/service_telemetry_probe.json`
- Service telemetry key fields: `verdict: ok_dream7b_bpu_selected_pair_cross_job_candidate_service_telemetry_probe`, `processed_request_count: 192`, `failed_job_count: 0`, `load_to_run_ratio: 8.66679`, `avg_bpu_loading: 10.108`, `max_bpu_loading: 98.0`, `amortized_wall_ms_per_processed_request: 1430.794`, and `selected_pair_covers_all_segments: True`.
- Cross-job service promotion gate: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_service_promotion_gate_20260610-183109/cross_job_service_promotion_gate_probe.json`
- Promotion key fields: `verdict: ok_dream7b_bpu_selected_pair_cross_job_service_promotion_gate_probe`, `promotion_allowed: True`, `promotion_decision: ready_for_default_service_replacement`, `default_service_replaced: False`, `promotion_blockers: []`, `processed_request_count: 192`, `load_to_run_ratio: 8.66679`, `avg_bpu_loading: 10.108`, and `amortized_wall_ms_per_processed_request: 1430.794`.
- Refreshed final gate: `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-183252/default_deployable_acceptance_probe.json` records `default_deployable_ready: False`, `default_deployable_status: ready_for_default_replacement_candidate_only`, `cross_job_service_candidate.performance_pass_count: 3`, `cross_job_service_candidate.ready_for_default_replacement: True`, `cross_job_service_candidate.default_service_replaced: False`, and `cross_job_service_candidate.default_replaced_and_ready: False`.
- Decision: this is the first Dream 7B S100P route with 192-request service telemetry that passes three performance gates: load/run below `9.443895`, average BPU above `8.811`, and long-stable wall time below `1451.906`. It is ready for controlled default replacement, but the goal is not complete because `default_service_replaced: False` and the candidate currently waits for at least two queued jobs before executing.

## 2026-06-10 Single-Job Fallback Flush

- Fallback probe: `scripts/probes/dream7b_bpu_selected_pair_cross_job_service_fallback_probe.sh`
- Fallback report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_service_fallback_20260610-185916/cross_job_service_fallback_probe.json`
- Fallback key fields: `verdict: ok_dream7b_bpu_selected_pair_cross_job_service_fallback_probe`, `single_job_fallback_ok: True`, `run_reason: single_job_flush_timeout`, `processed_request_count: 1`, `failed_job_count: 0`, `load_to_run_ratio: 155.028327`, and `amortized_wall_ms_per_processed_request: 20577.422`.
- Fallback-aware promotion gate: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_service_promotion_gate_20260610-185941/cross_job_service_promotion_gate_probe.json`
- Promotion key fields: `promotion_allowed: True`, `promotion_blockers: []`, and `single_job_fallback_ok: True`.
- Refreshed final gate: `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-185941/default_deployable_acceptance_probe.json` records `default_deployable_status: ready_for_default_replacement_candidate_only` because `default_service_replaced: False`.

## 2026-06-10 Default Service Promotion Closure

- Promotion probe: `scripts/probes/dream7b_bpu_cross_job_default_promotion_probe.sh`
- Promoted runtime: `/mnt/nas/openclaw/runtimes/dream7b-bpu-cross-job-default`
- Promotion report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_default_promotion_20260610-190712/cross_job_default_promotion_probe.json`
- Promotion key fields: `verdict: ok_dream7b_bpu_cross_job_default_promotion_probe`, `default_service_replaced: True`, `rollback_verified: True`, and `errors: []`.
- Default service telemetry probe: `scripts/probes/dream7b_bpu_cross_job_default_service_telemetry_probe.sh`
- Default service telemetry report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_default_service_telemetry_20260610-191115/default_service_telemetry_probe.json`
- Default service telemetry key fields: `verdict: ok_dream7b_bpu_cross_job_default_service_telemetry_probe`, `processed_request_count: 192`, `failed_job_count: 0`, `queue_done_count: 12`, `queue_failed_count: 0`, `load_to_run_ratio: 8.734653`, `avg_bpu_loading: 9.915`, `max_bpu_loading: 98.0`, and `amortized_wall_ms_per_processed_request: 1441.545`.
- Replacement-time deployment acceptance: `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-192303/deployment_acceptance_probe.json`
- Deployment key fields: `verdict: ok_dream7b_bpu_deployment_acceptance_probe`, `check_count: 30`, `passed_check_count: 30`, and `errors: []`.
- Final acceptance: `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-192303/default_deployable_acceptance_probe.json`
- Final acceptance key fields: `default_deployable_ready: True`, `default_deployable_status: ready`, `cross_job_default_service.performance_pass_count: 3`, `cross_job_service_candidate.default_replaced_and_ready: True`, `cross_job_service_candidate.rollback_verified: True`, `blockers: []`, and `warnings: []`.
- Decision: Dream 7B has moved from candidate-only to default service on S100P. The 128TOPS wording remains utilization-progress wording, not a max-loading claim, because the accepted evidence is average BPU, load/run, and sustained telemetry improvement.

## 2026-06-11 Dream 7B 50 Percent Utilization Goal And Phase 1 Preflight

- Goal document: `docs/dream7b_s100p_50pct_utilization_goal_2026-06-11.md`
- Current default-service baseline: `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_default_service_telemetry_20260611-150155/default_service_telemetry_probe.json`
- Current default-service key fields: `processed_request_count: 192`, `failed_job_count: 0`, `avg_bpu_loading: 9.695`, `max_bpu_loading: 100.0`, `load_to_run_ratio: 8.817409`, and `amortized_wall_ms_per_processed_request: 1460.823`.
- Current selected-pair cross-job retest: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_telemetry_20260611-152221/cross_job_queue_telemetry_probe.json`
- Current selected-pair key fields: `processed_request_count: 192`, `failed_job_count: 0`, `avg_bpu_loading: 9.764`, `max_bpu_loading: 100.0`, `load_to_run_ratio: 8.757816`, and `amortized_wall_ms_per_processed_request: 1443.045`.
- Current utilization diagnosis: `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260611-151217/utilization_gap_probe.json` keeps `diagnosis: hbm_reload_dominated`.
- Planner probe: `scripts/probes/dream7b_bpu_phase1_reload_experiment_planner_probe.sh`
- Planner report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_phase1_reload_experiment_planner_20260611-153934/phase1_reload_experiment_planner_probe.json`
- Planner decision: `recommended_next_experiment: phase1_topload_resplit_compile`; the no-new-HBM selected-pair shortcuts `[0,1]` and `[1,2]` do not cover all fine-adjacent segments through successful triplets.
- Phase 1 preflight probe: `scripts/probes/dream7b_bpu_phase1_segment_plan_preflight_probe.sh`
- Deployed preflight report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_phase1_segment_plan_preflight_20260611-154955/phase1_segment_plan_preflight_probe.json`
- Deployed dry-run report: `/mnt/nas/openclaw/reports/models/dream7b_phase1_deployed_dryrun/segment_plan_preflight.json`
- Phase 1 plan state: `phase1-topload-adjacent` is wired into the deployed forward path and dry-run verified without loading BPU runtimes, but `phase1_ready_to_run: False` because `seg02_03`, `seg03_04`, `seg04_05`, and `seg05_07` HBM artifacts are missing.
- Long-session telemetry probe: `scripts/probes/dream7b_bpu_cross_job_long_session_telemetry_probe.sh`
- Long-session 18x16 report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_long_session_telemetry_20260611-160959/long_session_telemetry_probe.json`
- Long-session 18x16 key fields: `processed_request_count: 288`, `failed_job_count: 0`, `avg_bpu_loading: 9.775`, `max_bpu_loading: 98.0`, `load_to_run_ratio: 8.711317`, `amortized_wall_ms_per_processed_request: 1450.312`, and `phase1_gate.passed: False`.
- Long-session 24x16 report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_long_session_telemetry_20260611-161823/long_session_telemetry_probe.json`
- Long-session 24x16 key fields: `processed_request_count: 384`, `failed_job_count: 0`, `avg_bpu_loading: 9.696`, `max_bpu_loading: 98.0`, `load_to_run_ratio: 8.741708`, `amortized_wall_ms_per_processed_request: 1448.516`, and `phase1_gate.passed: False`.
- Phase 1 compile recovery package probe: `scripts/probes/dream7b_bpu_phase1_compile_recovery_package_probe.sh`
- Phase 1 compile recovery package report: `/mnt/nas/openclaw/reports/models/dream7b_bpu_phase1_compile_recovery_package_20260611-164453/phase1_compile_recovery_package_probe.json`
- Phase 1 compile recovery runbook: `/mnt/nas/openclaw/reports/models/dream7b_bpu_phase1_compile_recovery_package_20260611-164453/phase1_compile_recovery_runbook.sh`
- Phase 1 compile recovery state: `verdict: ok_dream7b_bpu_phase1_compile_recovery_package_probe`, `errors: []`, Dream HF assets and the S100 LLM SDK are present on NAS, and the remaining execution dependency is an x86_64 Linux host with AVX/HBDK compiler import support. The S100P itself reports `machine: aarch64`, `host_is_builder: False`, and cannot compile the missing Phase 1 HBM shards.
- Phase 1 top-load compile result: `/mnt/nas/openclaw/reports/models/dream7b_resplit_compile_20260611-104601/resplit_compile_probe.json` records `verdict: ok_dream7b_resplit_compile_probe`, `specs: ['2:3', '3:4', '4:5', '5:7']`, `hbm_success_count: 4`, `failed_spec_count: 0`, and `skipped_existing_count: 1`. The generated HBM files were published to `/mnt/nas/openclaw/models/dream7b-hbm/phase1-topload-seq16`, and `sha256sum -c manifest.sha256` verifies `seg02_03`, `seg03_04`, `seg04_05`, and `seg05_07`.
- Phase 1 top-load runtime result: `/mnt/nas/openclaw/reports/models/dream7b_bpu_phase1_segment_plan_preflight_20260611-190121/phase1_segment_plan_preflight_probe.json` records `phase1_ready_to_run: True`, `missing_segment_count: 0`, and `errors: []`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260611-190500/resplit_batch_telemetry_probe.json` records `verdict: ok_dream7b_bpu_resplit_batch_telemetry_probe`, `segment_plan: phase1-topload-adjacent`, `segment_event_count: 320`, `avg_bpu_loading: 6.706`, `max_bpu_loading: 100.0`, `load_to_run_ratio: 12.012308`, and `amortized_wall_ms_per_forward: 2024.617`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260611-190552/resplit_window_cost_probe.json` records the same `load_to_run_ratio: 12.012308`, while `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260611-190552/utilization_gap_probe.json` keeps `diagnosis: hbm_reload_dominated`.
- Phase 1 decision: `phase1-topload-adjacent` is runnable and proves the full Dream shard compile-to-runtime loop, but it is rejected as a utilization regression because it is worse than the current default-service baseline around `avg_bpu_loading: 9.7%` and `load_to_run_ratio: 8.7`.
- Large-batch selected-pair 50% candidate: `scripts/probes/dream7b_bpu_50pct_candidate_acceptance_probe.sh`
- Large-batch selected-pair 50% telemetry: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_telemetry_20260611-192528/cross_job_queue_telemetry_probe.json`
- Large-batch selected-pair 50% acceptance: `/mnt/nas/openclaw/reports/models/dream7b_bpu_50pct_candidate_acceptance_20260611-193007/50pct_candidate_acceptance_probe.json`
- Large-batch selected-pair 50% key fields: `job_count: 2`, `request_count: 192`, `processed_request_count: 384`, `failed_job_count: 0`, `avg_bpu_loading: 52.328`, `max_bpu_loading: 98.0`, `load_to_run_ratio: 0.778725`, `amortized_wall_ms_per_processed_request: 262.083`, `deployment_acceptance_passed_check_count: 30`, `deployment_acceptance_check_count: 30`, `rollback_status: rollback_safe_candidate_only`, and `default_service_replaced: False`.
- Large-batch progression evidence: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_telemetry_20260611-191337/cross_job_queue_telemetry_probe.json` records `6x32`, `processed_request_count: 192`, `avg_bpu_loading: 17.087`, and `load_to_run_ratio: 4.448624`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_telemetry_20260611-191722/cross_job_queue_telemetry_probe.json` records `3x64`, `processed_request_count: 192`, `avg_bpu_loading: 28.813`, and `load_to_run_ratio: 2.279817`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_telemetry_20260611-192040/cross_job_queue_telemetry_probe.json` records `2x96`, `processed_request_count: 192`, `avg_bpu_loading: 37.074`, and `load_to_run_ratio: 1.5317`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_telemetry_20260611-192238/cross_job_queue_telemetry_probe.json` records `2x128`, `processed_request_count: 256`, `avg_bpu_loading: 43.426`, and `load_to_run_ratio: 1.157432`.
- Live rollback backups: `/usr/local/bin/dream7b-bpu-forward.before-phase1-20260611-154904`, `/usr/local/bin/dream7b-bpu-resplit-batch-forward.before-phase1-20260611-154904`, `/usr/local/bin/dream7b-bpu-resplit-forward.before-phase1-20260611-154904`, and `/mnt/nas/openclaw/runtimes/dream7b-bpu-forward/dream7b_segmented_hbm_python_forward.py.before-phase1-20260611-154904`.
- Decision: the long-run target is sustained `avg_bpu_loading >= 50%` on at least 192 requests, but the current verified level is about `9.7%`. Extending selected-pair resident sessions from 12 jobs to 18/24 jobs is stable but does not pass Phase 1, so job-count-only scaling is a minor tuning path, not a utilization breakout. The next executable optimization step is compiling and publishing the four missing Phase 1 top-load HBM shards on an approved x86 Linux HBDK builder, then running batch telemetry, window-cost, utilization-gap, deployment acceptance, and default-deployable acceptance before any utilization-progress claim.
## Dream 7B Long-Running Candidate 2026-06-11
...
Candidate soak probe:

```text
scripts/probes/dream7b_bpu_50pct_candidate_soak_probe.sh
```

Final long-running candidate acceptance probe:
...
```text
DREAM7B_BPU_50PCT_SOAK_TARGET_DURATION_SEC=1800
DREAM7B_BPU_50PCT_SOAK_JOB_COUNT=2
DREAM7B_BPU_50PCT_SOAK_REQUEST_COUNT=192
DREAM7B_BPU_50PCT_SOAK_REQUEST_COUNT_LIMIT=256
```

Completed 30-minute soak evidence:

```text
/mnt/nas/openclaw/reports/models/dream7b_50pct_soak_launcher_20260611-224510.log
/mnt/nas/openclaw/reports/models/dream7b_bpu_50pct_candidate_soak_20260611-224510
```
...
50% service candidate plan:

```text
service_name: dream7b-bpu-selected-pair-cross-job-candidate-50pct.service
...
OpenClaw Personal NAS dry-run tool:
...
## Dream 7B 90 Percent Utilization Route 2026-06-12
...
Installed 50% service candidate:
...
max_batch_size_limit: 256
default_service_replaced: false
rollback: sudo systemctl disable --now dream7b-bpu-selected-pair-cross-job-candidate-50pct.service
```

Installed segment-major/load-once service candidate:

```text
dream7b-bpu-segment-major-load-once-candidate.service
queue_dir: /mnt/nas/openclaw/queues/dream7b-bpu-segment-major-load-once-candidate
output_dir: /mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_load_once_candidate_service
max_batch_size: 192
max_batch_size_limit: 256
default_service_replaced: false
rollback: sudo systemctl disable --now dream7b-bpu-segment-major-load-once-candidate.service
```

Installed 24-job segment-major/load-once service candidate:
...
max_job_count: 24
max_job_count_limit: 24
max_batch_size: 192
max_batch_size_limit: 256
default_service_replaced: false
rollback: sudo systemctl disable --now dream7b-bpu-segment-major-load-once-candidate-24job.service
```

Installed 24x256 segment-major/load-once service candidate:
...
max_batch_size_limit: 256
default_service_replaced: false
rollback: sudo systemctl disable --now dream7b-bpu-segment-major-load-once-candidate-24job-b256.service
```

Service telemetry probes:
...
Backlog sweep summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_50pct_service_backlog_sweep_summary_20260612-112113/50pct_service_backlog_sweep_summary_probe.md
verdict: ok_dream7b_bpu_50pct_service_backlog_sweep_summary_probe
decision: backlog_plateau_below_70_percent
best_job_count: 12
best_avg_bpu_loading: 53.459
best_load_to_run_ratio: 0.734513
plateau_delta_avg_bpu_last_two_job_counts: 0.043
```
...
Continuous prefetch summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_50pct_candidate_service_continuous_prefetch_20260612-121446/50pct_candidate_service_continuous_prefetch_probe.md
...
Segment-major load-once forward summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_load_once_forward_20260612-124719/segment_major_load_once_forward_probe.md
...
load_to_run_ratio_delta_vs_baseline: -0.663232
...
Segment-major service telemetry summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_service_telemetry_20260612-130705/segment_major_candidate_service_telemetry_probe.md
...
Segment-major pilot soak summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_soak_20260612-131931/segment_major_candidate_soak_probe.md
...
Segment-major 30-minute soak summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_soak_20260612-133613/segment_major_candidate_soak_probe.md
verdict: ok_dream7b_bpu_segment_major_candidate_soak_probe
decision: segment_major_service_soak_near_90pct_candidate
elapsed_sec: 1892
iteration_count: 5
...
Segment-major deep aggregation summaries:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_load_once_forward_20260612-141202/segment_major_load_once_forward_probe.md
job_count: 16
...
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_load_once_forward_20260612-142048/segment_major_load_once_forward_probe.md
job_count: 24
...
Segment-major 24-job service telemetry summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_service_telemetry_20260612-143609/segment_major_candidate_service_telemetry_probe.md
verdict: ok_dream7b_bpu_segment_major_candidate_service_telemetry_probe
decision: segment_major_service_near_90pct_candidate
...
Segment-major 24-job pilot soak summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_soak_20260612-144904/segment_major_candidate_soak_probe.md
verdict: ok_dream7b_bpu_segment_major_candidate_soak_probe
decision: segment_major_service_soak_near_90pct_candidate
elapsed_sec: 1461
iteration_count: 2
...
Segment-major 24x224 and 24x256 service telemetry:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_service_telemetry_20260612-155626/segment_major_candidate_service_telemetry_probe.md
...
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_service_telemetry_20260612-161133/segment_major_candidate_service_telemetry_probe.md
...
Segment-major 24x256 pilot soak summary:

```text
/mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_soak_20260612-162823/segment_major_candidate_soak_probe.md
verdict: ok_dream7b_bpu_segment_major_candidate_soak_probe
...
```text
Backlog depth and continuous outer-queue prefetch are not enough for 70-80%
average BPU, but segment-major/load-once scheduling crosses the ratio gate in
both offline and service form. The current service candidate reaches 86.792%
average BPU with load_to_run_ratio 0.068115 on 12x192, and the pilot soak
holds 86.701% average BPU across two consecutive iterations. The 30-minute soak
holds 86.643% average BPU across five consecutive iterations and 11520
requests. The 24-job candidate reaches 89.535% service telemetry and 89.471%
two-iteration pilot soak with zero failed jobs. The 24x256 candidate reaches
90.275% service telemetry, 90.327% two-iteration pilot soak, and a 2-hour soak
at 90.264% average BPU with zero failed jobs. The current utilization goal is
met as a rollback-safe default candidate; remaining work is operator promotion
and a vendor question about making this scheduling pattern native in the
official Dream adapter/HBM layout/runtime memory-pool route.
```

OpenClaw local Dream validation:
...
/mnt/nas/openclaw/reports/models/dream7b_openclaw_candidate_validation_20260612-172108/openclaw_candidate_validation_probe.md
...
two_hour_soak_report: /mnt/nas/openclaw/reports/models/dream7b_bpu_segment_major_candidate_soak_20260612-171402/segment_major_candidate_soak_probe.json
verdict: ok_dream7b_bpu_segment_major_candidate_soak_probe
decision: segment_major_service_soak_meets_90pct_goal
elapsed_sec: 7723
iteration_count: 8
...
Default-service promotion and ops hardening:
...
--max-batch-size 256
errors: []
scripts/dream7b-default-status
scripts/dream7b-default-rollback
...
OpenClaw Dream local NAS copy-sort demo:
...
delete_or_move_performed: False
copy_mode: copy_to_personal_sorted_preserve_originals
Sorted/Movies/Sci-Fi
Sorted/Movies/Action
```
