...
2026-06-09 round3 top-window update: the next targeted split compiled and published `seg14_15`, `seg15_17`, `seg17_18`, and `seg18_19` into both `/mnt/nas/openclaw/models/dream7b-hbm/resplit-topwindow-seq16` and `/home/sunrise/.cache/openclaw/dream7b-hbm/resplit-topwindow-seq16`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_hbm_artifact_inventory_20260609-152820/resplit_hbm_artifact_inventory_probe.json` verifies the NAS copy and `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_hbm_artifact_inventory_20260609-152823/resplit_hbm_artifact_inventory_probe.json` verifies the S100P local cache; both record `expected_specs: ['7:8', '8:10', '14:15', '15:17', '17:18', '18:19', '21:22', '22:24']`, `expected_hbm_count: 8`, `existing_hbm_count: 8`, `manifest_entry_count: 8`, `manifest_verified_count: 8`, `total_hbm_size_bytes: 2512771264`, and `errors: []`. The runtime `RESPLIT_TOPWINDOW_ADJACENT_SEGMENTS` candidate now replaces `seg14_17` with `seg14_15 + seg15_17` and replaces `seg17_19` with `seg17_18 + seg18_19`.

2026-06-09 round3 runtime update: `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260609-153118/resplit_batch_telemetry_probe.json` records `verdict: ok_dream7b_bpu_resplit_batch_telemetry_probe`, `batch_count: 16`, `expected_segment_plan: resplit-topwindow-adjacent`, `forward_metrics.segment_event_count: 288`, `forward_metrics.segment_sources: ['base', 'fine', 'resplit', 'topwindow']`, `max_bpu_loading: 100.0`, `avg_bpu_loading: 9.628`, `forward_metrics.load_ms: 23166.108`, `forward_metrics.run_ms: 2446.735`, and `forward_metrics.load_to_run_ratio: 9.468172`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260609-153244/resplit_window_cost_probe.json` records `verdict: ok_dream7b_bpu_resplit_window_cost_probe`, `segment_plan: resplit-topwindow-adjacent`, `segment_event_count: 288`, `window_count: 9`, `load_to_run_ratio: 9.468172`, top absolute-load window `['seg02_04', 'seg04_07']`, and top load/run-ratio window `['seg00_01', 'seg01_02']`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260609-153933/utilization_gap_probe.json` records `verdict: ok_dream7b_bpu_utilization_gap_probe`, `diagnosis: hbm_reload_dominated`, `resplit_batch_telemetry.load_to_run_ratio: 9.468`, and `resplit_batch_telemetry.avg_bpu_loading: 9.628`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260609-154013/deployment_acceptance_probe.json` records `verdict: ok_dream7b_bpu_deployment_acceptance_probe`, `check_count: 30`, `passed_check_count: 30`, and `utilization_gap.ok: True`. This improves the previous top-window `load_to_run_ratio: 9.694618`, but the route remains HBM-reload dominated and must not be reported as a sustained 128TOPS utilization solution.
...
2026-06-09 teacher-guided fullflow baseline update: the next Qwen/Dream comparison is now encoded as two reusable probes rather than an ad hoc manual attempt. `scripts/probes/s100_official_qwen_fullflow_probe.sh` fixes the official baseline to `model_name: qwen2_5-1_5b`, `march: nash-m`, `cache_len: 1024`, `chunk_size: 256`, output root `/mnt/nas/openclaw/models/s100-official-qwen-fullflow`, and report root `/mnt/nas/openclaw/reports/models/s100_official_qwen_fullflow_*`. It creates `calib_prompts.json`, runs `oellm_build` only on a compatible x86_64 AVX HBDK host, writes an isolated `qwen_fullflow_config.json`, and captures `oellm_multichat` stdout/stderr plus optional `hrt_ucp_monitor` telemetry without overwriting the vendor `qwen_multichat_config.json`. This is a method baseline and fallback candidate only; it does not replace Dream 7B as the target.

2026-06-09 Dream OELLM migration gate: `scripts/probes/dream7b_oellm_fullflow_feasibility_probe.sh` records whether Dream appears in the official `leap_llm` model registry, summarizes `Dream` config fields, then reruns the existing `scripts/probes/compile_dream_with_deepseek_skeleton.sh` route using the same SDK/build-host assumptions (`DREAM7B_OELLM_FULLFLOW_MARCH=nash-m`, `DREAM7B_OELLM_FULLFLOW_CHUNK_SIZE=256`, `DREAM7B_OELLM_FULLFLOW_CACHE_LEN=1024`, `DREAM7B_OELLM_FULLFLOW_W_BITS=8`). If the direct route fails, the report includes `failure_stage`, command, environment, SDK registry, Dream config summary, and full stdout/stderr paths. The segmented Dream HBM route remains the main path unless this feasibility probe produces non-empty `.hbm` artifacts and later S100P runtime telemetry.

2026-06-09 fullflow run result: after staging `Qwen/Qwen2.5-1.5B-Instruct` through the HuggingFace mirror, the x86_64 AVX WSL build report `/mnt/nas/openclaw/reports/models/qwen_fullflow_wsl_build_report_20260609-081247/official_qwen_fullflow_probe.json` records `source_model_exists: true`, `build_host_compatible: true`, `host_machine: x86_64`, `compiled_hbm_path: /mnt/f/Project/Digua/tmp/models/s100-official-qwen-fullflow/qwen2_5-1_5b_chunk_256_cache_1024_q8.hbm`, and `compiled_hbm_size_bytes: 1917926648`. `oellm_build` still returns `1` after emitting the HBM because WSL host-side HBM load/link verification fails with `Cannot malloc bpu memory ... ION_IOC_ALLOC(ret=No such device)`, which is expected for a non-S100P host. The HBM and Qwen HF source were then published to `/mnt/nas/openclaw/models/s100-official-qwen-fullflow/qwen2_5-1_5b_chunk_256_cache_1024_q8.hbm` and `/mnt/nas/openclaw/models/qwen2_5-1_5b-hf`; `/mnt/nas/openclaw/reports/models/s100_official_qwen_fullflow_20260609-172710/official_qwen_fullflow_probe.json` records `source_model_exists: true`, `compiled_hbm_size_bytes: 1917926648`, `isolated_runtime_config_written: true`, `runtime_attempted: true`, `hbm_load_success_observed: true`, `init_model_success_observed: true`, `memory_alloc_failure_observed: true`, and `runtime_returncode: -11`. The official Qwen fullflow has therefore moved from missing-HBM/build-host preflight to S100P runtime blocked by BPU/common-buffer allocation. `/mnt/nas/openclaw/reports/models/dream7b_oellm_fullflow_feasibility_20260609-152748/dream7b_oellm_fullflow_feasibility_probe.json` records `dream_registered_in_official_sdk: false`, `build_host_compatible: false`, `compile_status: blocked_preflight`, `direct_oellm_migration_supported: false`, and `failure_stage: preflight`; its minimal failure package includes the Dream config summary, SDK registry summary, and stdout/stderr paths.

2026-06-09 current Qwen/Dream migration acceptance update: the official Qwen method baseline is now Qwen2.5-1.5B with `cache_len: 512` and `chunk_size: 128`, not the high-context 1024/256 case. The runnable HBM is `/mnt/nas/openclaw/models/s100-official-qwen-fullflow/cache_len_512_chunk_128/qwen2_5-1_5b_chunk_128_cache_512_q8.hbm`, and `/mnt/nas/openclaw/reports/models/s100_official_qwen_fullflow_20260609-210514/official_qwen_fullflow_probe.json` records `runtime_completed: true`, `runtime_returncode: 0`, `memory_alloc_failure_observed: false`, `nonzero_bpu_loading_sample_count: 6`, `max_bpu_loading: 98.0`, `avg_bpu_loading: 2.222`, and `Performance prefill: 1024.00tokens/s    decode: 25.57tokens/s`. `/mnt/nas/openclaw/reports/models/s100_qwen15_common_buffer_diagnosis_20260609-211000/s100_qwen15_common_buffer_diagnosis_20260609.md` keeps the BPU/common-buffer diagnosis and treats `/mnt/nas/openclaw/reports/models/s100_official_qwen_fullflow_20260609-185843/official_qwen_fullflow_probe.json` with 1024/256 as a high-context failure case (`runtime_returncode: -11`, `memory_alloc_failure_observed: true`, `AllocError { len: 2359296 }`), not as the Dream migration baseline.

2026-06-09 current Dream OELLM migration update: `/mnt/nas/openclaw/reports/models/dream7b_oellm_fullflow_feasibility_20260609-223754/dream7b_oellm_fullflow_feasibility_probe.json` uses the same 512/128 Qwen baseline assumptions and records `host_machine: x86_64`, `host_has_avx: true`, `build_host_compatible: true`, `dream_registered_in_official_sdk: false`, `supported_models_from_sdk: ['deepseek-qwen-1_5b', 'deepseek-qwen-7b', 'qwen2_5-1_5b', 'qwen2_5-7b', 'internvl2-2b', 'internvl2_5-2b', 'internvl2-1b', 'internvl2_5-1b', 'internlm2-1_8b', 'qwen2_5-omni-3b']`, `compile_status: blocked_registry_missing`, `failure_stage: registry_missing`, `compiled_hbm_count: 0`, `direct_oellm_migration_supported: false`, `missing_adapter_evidence.registry_missing: true`, `missing_adapter_evidence.required_adapter: official leap_llm model_factory registration and Dream/DreamModel adapter`, and `unable_to_attempt_direct_official_compile_reason`. The Dream source/config is `/mnt/f/Project/Digua/tmp/dream_hf/config.json`, with `model_type: Dream`, `architectures: ['DreamModel']`, `hidden_size: 3584`, `num_hidden_layers: 28`, `num_attention_heads: 28`, `num_key_value_heads: 4`, `vocab_size: 152064`, `mask_token_id: 151666`, `torch_dtype: bfloat16`, and `use_cache: true`. This is a registry/model-adapter gap in the official SDK route, not an environment or HBM corruption failure.

2026-06-09 current Dream segmented refresh: `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_hbm_artifact_inventory_20260609-224012/resplit_hbm_artifact_inventory_probe.json` and `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_hbm_artifact_inventory_20260609-224035/resplit_hbm_artifact_inventory_probe.json` both verify the round3 top-window split `14:15`, `15:17`, `17:18`, and `18:19` with `expected_hbm_count: 8`, `existing_hbm_count: 8`, `manifest_verified_count: 8`, and `total_hbm_size_bytes: 2512771264`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260609-224100/resplit_batch_telemetry_probe.json` records `expected_segment_plan: resplit-topwindow-adjacent`, `forward_metrics.segment_event_count: 288`, `avg_bpu_loading: 8.97`, and `forward_metrics.load_to_run_ratio: 9.678265`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260609-224142/resplit_window_cost_probe.json` records `window_count: 9` and `load_to_run_ratio: 9.678265`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260609-224156/utilization_gap_probe.json` records `diagnosis: hbm_reload_dominated`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260609-224157/deployment_acceptance_probe.json` records `check_count: 30` and `passed_check_count: 30`. This refresh improves the earlier `load_to_run_ratio: 9.694618` but does not improve on the current best `load_to_run_ratio: 9.468172`, so it must remain `hbm_reload_dominated` and cannot be described as sustained 128TOPS utilization success.

2026-06-10 official DeepSeek 7B fallback update: `scripts/probes/s100_official_deepseek7b_baseline_probe.sh` verifies the vendor DeepSeek-R1-Distill-Qwen-7B HBM path `/mnt/nas/openclaw/models/s100-official-deepseek-r1-distill-qwen-7b/DeepSeek_R1_Distill_Qwen_7B_1024.hbm` and runs an isolated `oellm_multichat` config without overwriting SDK examples or changing ION/performance-mode settings. `/mnt/nas/openclaw/reports/models/s100_official_deepseek7b_baseline_20260610-023455/deepseek7b_baseline_probe.json` records `hbm_exists: True`, `hbm_size_bytes: 7928846896`, `runtime_status: failed`, `runtime_returncode: 1`, `runtime_completed: False`, `hbm_load_success_observed: False`, `init_model_success_observed: False`, `memory_alloc_failure_observed: True`, `bpu_alloc_request_bytes: 7928846896`, and `decision: official_7b_runtime_blocked_common_buffer`. The official 7B fallback is therefore not runnable on the current S100P memory layout; Qwen2.5-1.5B `512/128` remains the verified official runnable fallback and is currently the only verified official fallback while Dream continues through segmented HBM optimization.

2026-06-10 reload optimization closure: `docs/dream7b_vendor_support_package_2026-06-10.md` packages the two external questions for teacher/vendor support: the S100P memory layout required to load `DeepSeek_R1_Distill_Qwen_7B_1024.hbm`, and whether Dream/DreamModel has an official `oellm_build/leap_llm` adapter path. `scripts/probes/dream7b_bpu_reload_optimization_probe.sh` then consolidates persistent cache, file prefetch/local-cache, selected-pair cache, window scheduling, and BPU core scheduling evidence. The fresh top-window run `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260610-030740/resplit_batch_telemetry_probe.json` records `avg_bpu_loading: 8.811`, `max_bpu_loading: 100.0`, `forward_metrics.load_to_run_ratio: 9.524202`, and `forward_metrics.wall_ms: 26007.248`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260610-030820/resplit_window_cost_probe.json` is the matching window-cost report; `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260610-030820/utilization_gap_probe.json` keeps `diagnosis: hbm_reload_dominated`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-030831/deployment_acceptance_probe.json` records `check_count: 30` and `passed_check_count: 30`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-030831/reload_optimization_probe.json` records `final_decision: keep_hbm_reload_dominated`, `substantial_improvement_observed: False`, `best_load_to_run_ratio: 9.468172`, and `ratio_delta_vs_best: 0.05603`. Because `9.524202` is worse than the current best `9.468172`, this run is not a 128TOPS utilization improvement.

2026-06-10 selected-pair candidate update: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_telemetry_20260610-031613/selected_pair_telemetry_probe.json` reruns the selected-pair resident path with `batch_count: 16` and records `selected_pair: [1, 8]`, `selected_pair_covers_all_segments: True`, `selected_pair.wall_ms: 22982.452`, `selected_pair.avg_bpu_loading: 8.984`, `selected_pair_wall_delta_ratio_vs_resplit: 0.116306`, and `selected_pair_avg_bpu_delta_vs_resplit: 0.173` versus the latest top-window wall time `26007.248` and average BPU loading `8.811`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260610-031818/utilization_gap_probe.json` still keeps `diagnosis: hbm_reload_dominated`, and `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-031818/deployment_acceptance_probe.json` records `check_count: 30` and `passed_check_count: 30`. The refreshed decision `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-031828/reload_optimization_probe.json` records `final_decision: utilization_progress_candidate`, `substantial_improvement_observed: True`, `best_load_to_run_ratio: 9.468172`, and `ratio_delta_vs_best: 0.05603`. This is a guarded candidate based on batch16 wall-time and average-BPU improvement, not a default-service promotion.

2026-06-10 sustained selected-pair service update: `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_candidate_service_telemetry_20260610-032523/systemd_telemetry_probe.json` submits 3x16 requests through `dream7b-bpu-selected-pair-candidate.service` and records `processed_request_count: 48`, `batch_counts: [16, 16, 16]`, `amortized_wall_ms_per_processed_request: 1445.673`, `avg_bpu_loading: 8.371`, `max_bpu_loading: 98.0`, `load_to_run_ratio: 9.859028`, `wall_ms_delta_ratio_vs_default_systemd: 0.130546`, and `candidate_avg_bpu_loading_not_worse_than_default_systemd: False`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_utilization_gap_20260610-032709/utilization_gap_probe.json` still reports `diagnosis: hbm_reload_dominated`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-032709/deployment_acceptance_probe.json` records `check_count: 30` and `passed_check_count: 30`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-032926/reload_optimization_probe.json` records `final_decision: guarded_sustained_wall_time_candidate`, `sustained_service_decision: guarded_sustained_wall_time_candidate`, `default_service_replacement_decision: do_not_replace_default_service_yet`, `wall_delta_ratio_vs_resplit_per_request: 0.110603`, `avg_bpu_delta_vs_resplit: -0.44`, and `avg_bpu_delta_vs_selected_pair_single: -0.613`. The service candidate is therefore promoted only to a rollback-gated wall-time candidate; it is not a default-service replacement and does not resolve the average BPU utilization gap.

2026-06-10 selected-pair service promotion gate: `scripts/probes/dream7b_bpu_selected_pair_service_promotion_gate_probe.sh` adds the default-service replacement gate for the sustained selected-pair service. `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_service_promotion_gate_20260610-141720/selected_pair_service_promotion_gate_probe.json` records `verdict: ok_dream7b_bpu_selected_pair_service_promotion_gate_probe`, `promotion_allowed: False`, `promotion_decision: block_default_service_replacement`, `default_service_replaced: False`, `candidate_service_isolated_from_default: True`, `candidate_load_to_run_ratio: 9.859028`, `wall_delta_ratio_vs_default_systemd: 0.130546`, `wall_delta_ratio_vs_resplit_per_request: 0.110603`, `avg_bpu_delta_vs_default_systemd: -1.245`, `avg_bpu_delta_vs_resplit: -0.44`, `utilization_diagnosis: hbm_reload_dominated`, and `promotion_blockers: ['promotion_average_bpu_not_worse_vs_default', 'promotion_average_bpu_improved_vs_resplit', 'promotion_load_to_run_not_worse_than_best', 'promotion_not_hbm_reload_dominated', 'reload_gate_allows_default_replacement']`. The embedded rollback includes `sudo systemctl restart dream7b-bpu-batch-queue.service`. This closes the promotion decision: the candidate service may stay active and isolated for testing, but the default Dream service must not be replaced until average BPU loading, load/run, and `hbm_reload_dominated` blockers are cleared.

2026-06-10 promotion blocker diagnosis: `scripts/probes/dream7b_bpu_promotion_blocker_diagnosis_probe.sh` consolidates the failed promotion gate into concrete next experiments. `/mnt/nas/openclaw/reports/models/dream7b_bpu_promotion_blocker_diagnosis_20260610-145739/promotion_blocker_diagnosis_probe.json` records `verdict: ok_dream7b_bpu_promotion_blocker_diagnosis_probe`, `promotion_decision: keep_candidate_only_until_blockers_clear`, `candidate_load_to_run_ratio: 9.859028`, `load_to_run_delta_vs_best: 0.390856`, `load_to_run_delta_vs_resplit: 0.334826`, `service_avg_bpu_loading: 8.371`, `avg_bpu_delta_vs_resplit: -0.44`, `wall_delta_ratio_vs_resplit_per_request: 0.110603`, `top_load_to_run_ratio_window.resident_segments: ['seg00_01', 'seg01_02']`, `top_load_to_run_ratio_window.load_to_run_ratio: 18.585073`, `top_load_window.resident_segments: ['seg02_04', 'seg04_07']`, `top_load_window.load_ms: 3562.79`, `max_resident_segment_count_observed: 3`, `successful_seeded_quad_count: 0`, and `recommended_resplit_segment_indexes: [0, 9, 4, 6]`. The next optimization priority is `prefix_micro_window_reload_reduction`, followed by `seg02_04_seg04_07_window_cost_reduction`; default replacement remains blocked.

2026-06-10 final optimization acceptance: `scripts/probes/dream7b_final_optimization_acceptance_probe.sh` aggregates the full Dream 7B deployment optimization evidence against the final acceptance criteria. `/mnt/nas/openclaw/reports/models/dream7b_final_optimization_acceptance_20260610-161744/final_optimization_acceptance_probe.json` records `verdict: ok_dream7b_final_optimization_acceptance_probe`, `final_goal_satisfied: True`, `final_decision: dream7b_bpu_optimized_candidate_complete_candidate_only`, `deployment_route_closed: True`, `substantial_improvement_observed: True`, `utilization_statement_compliant: True`, `promotion_gate_closed: True`, `dream_oellm_registry_missing: True`, `qwen_fallback_verified: True`, `deepseek7b_blocked_common_buffer: True`, `candidate_service_only: True`, `default_service_replaced: False`, `selected_pair_wall_delta_ratio_vs_resplit: 0.116306`, `sustained_wall_delta_ratio_vs_resplit_per_request: 0.110603`, `sustained_wall_delta_ratio_vs_default_systemd: 0.130546`, and `sustained_request_count: 48`. This final acceptance closes the current objective as a candidate-only BPU optimization route: it proves a measured wall-time improvement and full rollback-gated evidence, while preserving the conclusion that average BPU and load/run still block default-service replacement.

2026-06-10 normal-use candidate acceptance: `scripts/probes/dream7b_bpu_normal_use_acceptance_probe.sh` adds the stricter normal-use gate for the selected-pair service. `/mnt/nas/openclaw/reports/models/dream7b_bpu_normal_use_acceptance_20260610-170005/normal_use_acceptance_probe.json` records `verdict: ok_dream7b_bpu_normal_use_acceptance_probe`, `normal_use_ready: True`, `unmet_requirements: []`, `stable_sustained_service: True`, `preferred_96_request_sustained_observed: True`, `deployment_acceptance_clean: True`, `performance_floor_met: True`, `reload_relief_observed: True`, `deployable_rollback_safe: True`, and `utilization_claim_compliant: True`. The current 48-request service evidence `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_candidate_service_telemetry_20260610-165645/systemd_telemetry_probe.json` records `processed_request_count: 48`, `batch_counts: [16, 16, 16]`, `failed_job_count: 0`, `amortized_wall_ms_per_processed_request: 1448.877`, `avg_bpu_loading: 8.578`, `max_bpu_loading: 96.0`, `wall_delta_ratio_vs_resplit_per_request: 0.102517`, `wall_delta_ratio_vs_default_systemd: 0.128619`, and `load_to_run_ratio: 9.863791`; the preferred 96-request stability evidence is `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_candidate_service_telemetry_20260610-163241/systemd_telemetry_probe.json`. The post-blocker reload experiment `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_batch_telemetry_20260610-165429/resplit_batch_telemetry_probe.json` and `/mnt/nas/openclaw/reports/models/dream7b_bpu_resplit_window_cost_20260610-165513/resplit_window_cost_probe.json` record `forward_metrics.load_to_run_ratio: 9.443895`, `load_to_run_ratio: 9.443895`, previous `9.774037`, `top_ratio_improved: True`, `top_load_improved: True`, and `overall_load_to_run_improved: True`. This reaches the normal-use candidate bar, but it remains `hbm_reload_dominated` and candidate-only because sustained average BPU loading and load/run still do not clear the default-service promotion gate.

2026-06-10 default-deployable final acceptance: `scripts/probes/dream7b_bpu_default_deployable_acceptance_probe.sh` adds the stricter final gate for replacing the default Dream service. `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-173758/default_deployable_acceptance_probe.json` records `verdict: ok_dream7b_bpu_default_deployable_acceptance_probe`, `default_deployable_ready: False`, `default_deployable_status: blocked_candidate_only`, `processed_request_count: 192`, `failed_job_count: 0`, `preferred_sustained_observed: True`, `deployment_acceptance_clean: True`, `promotion_allowed: False`, `performance_pass_count: 1`, `min_performance_pass_count: 3`, `amortized_wall_ms_per_processed_request: 1450.4`, `avg_bpu_loading: 8.476`, and `load_to_run_ratio: 9.873488`. The 192-request sustained service therefore proves long-run stability, but it does not beat the default-deploy performance gate. The same report lists `service_performance_pass_count_below_3`, `service_avg_bpu_below_default_deploy_threshold`, and `service_load_to_run_above_default_deploy_threshold`, and it carries forward promotion blockers from `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_service_promotion_gate_20260610-173049/selected_pair_service_promotion_gate_probe.json`: `promotion_average_bpu_not_worse_vs_default`, `promotion_average_bpu_improved_vs_resplit`, `promotion_load_to_run_not_worse_than_best`, `promotion_not_hbm_reload_dominated`, and `reload_gate_allows_default_replacement`. The matching reload decision `/mnt/nas/openclaw/reports/models/dream7b_bpu_reload_optimization_20260610-173049/reload_optimization_probe.json` keeps `default_service_replacement_decision: do_not_replace_default_service_yet` and `utilization_diagnosis: hbm_reload_dominated`. Cross-job reuse at `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_reuse_20260610-172408/selected_pair_cross_job_reuse_probe.json` is the best remaining optimization lead: it records `processed_forward_count: 128`, `load_to_run_ratio: 8.895442`, `amortized_wall_ms_per_forward: 1448.768`, `load_ms_delta_ratio_vs_service: 0.10225`, and `cross_job_wall_time_improved: True`, but it remains a prototype rather than a systemd service and only reaches `performance_pass_count: 2`. The current final conclusion is therefore candidate-only normal-use readiness, not default deployment.

2026-06-10 cross-job queue runner candidate: `scripts/dream7b_bpu_selected_pair_cross_job_queue_runner.py` turns the cross-job reuse idea into a real queue-runner candidate by draining multiple pending JSONL jobs through one selected-pair resident worker session. `scripts/probes/dream7b_bpu_selected_pair_forward_path_probe.sh` now accepts `DREAM7B_BPU_SELECTED_PAIR_TOKENS_BATCHES_BY_JOB_JSON` and permits `DREAM7B_BPU_SELECTED_PAIR_JOB_COUNT` up to 12 for this selected-only path. `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_runner_20260610-175302/cross_job_queue_summary.json` records the 6x16 result: `verdict: ok_dream7b_bpu_selected_pair_cross_job_queue_runner`, `processed_job_count: 6`, `processed_request_count: 96`, `failed_job_count: 0`, `load_to_run_ratio: 8.890613`, `amortized_wall_ms_per_processed_request: 1438.937`, `amortized_total_load_ms_per_processed_request: 1309.349`, `amortized_run_ms_per_processed_request: 147.273`, and `selected_pair_covers_all_segments: True`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_queue_runner_20260610-175739/cross_job_queue_summary.json` records the preferred 12x16 result: `processed_job_count: 12`, `processed_request_count: 192`, `failed_job_count: 0`, `load_to_run_ratio: 8.904793`, `amortized_wall_ms_per_processed_request: 1459.145`, and `selected_pair_covers_all_segments: True`. The refreshed gates `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-180307/deployment_acceptance_probe.json`, `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_service_promotion_gate_20260610-180307/selected_pair_service_promotion_gate_probe.json`, and `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-180307/default_deployable_acceptance_probe.json` record `errors: []`, `promotion_allowed: False`, `promotion_decision: block_default_service_replacement`, `default_deployable_ready: False`, `default_deployable_status: blocked_candidate_only`, `cross_job_queue_candidate.preferred_sustained_observed: True`, `cross_job_queue_candidate.performance_pass_count: 1`, and `cross_job_queue_candidate.load_to_run_ratio: 8.904793`. This is real progress on HBM reload reduction, but it is not enough for default deployment because 192-request wall time regresses and average BPU is still not proven improved.

2026-06-10 cross-job candidate service promotion gate: `scripts/install_dream7b_bpu_selected_pair_cross_job_candidate_service.sh` installs `dream7b-bpu-selected-pair-cross-job-candidate.service`, and `scripts/dream7b_bpu_selected_pair_cross_job_queue_service.py` runs the cross-job queue loop as an isolated systemd candidate. Supporting probes are `scripts/probes/dream7b_bpu_selected_pair_cross_job_queue_telemetry_probe.sh` and `scripts/probes/dream7b_bpu_selected_pair_cross_job_service_promotion_gate_probe.sh`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_candidate_service_telemetry_20260610-182409/service_telemetry_probe.json` records `verdict: ok_dream7b_bpu_selected_pair_cross_job_candidate_service_telemetry_probe`, `processed_request_count: 192`, `failed_job_count: 0`, `load_to_run_ratio: 8.66679`, `avg_bpu_loading: 10.108`, `max_bpu_loading: 98.0`, `amortized_wall_ms_per_processed_request: 1430.794`, and `selected_pair_covers_all_segments: True`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_service_promotion_gate_20260610-183109/cross_job_service_promotion_gate_probe.json` records `verdict: ok_dream7b_bpu_selected_pair_cross_job_service_promotion_gate_probe`, `promotion_allowed: True`, `promotion_decision: ready_for_default_service_replacement`, `default_service_replaced: False`, `promotion_blockers: []`, `processed_request_count: 192`, `load_to_run_ratio: 8.66679`, `avg_bpu_loading: 10.108`, and `amortized_wall_ms_per_processed_request: 1430.794`. The refreshed final acceptance `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-183252/default_deployable_acceptance_probe.json` records `default_deployable_ready: False`, `default_deployable_status: ready_for_default_replacement_candidate_only`, `cross_job_service_candidate.performance_pass_count: 3`, `cross_job_service_candidate.ready_for_default_replacement: True`, `cross_job_service_candidate.default_service_replaced: False`, and `cross_job_service_candidate.default_replaced_and_ready: False`. This clears the performance and promotion gate for a controlled replacement candidate, but the final default-deploy goal remains open because the default service has not been replaced and single-job behavior still needs a controlled fallback or timeout policy.

2026-06-10 single-job fallback flush update: `scripts/probes/dream7b_bpu_selected_pair_cross_job_service_fallback_probe.sh` verifies that the cross-job candidate service can drain one pending request through `--single-job-flush-timeout-sec` instead of waiting indefinitely for a second job. `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_service_fallback_20260610-185916/cross_job_service_fallback_probe.json` records `verdict: ok_dream7b_bpu_selected_pair_cross_job_service_fallback_probe`, `single_job_fallback_ok: True`, `run_reason: single_job_flush_timeout`, `processed_request_count: 1`, `failed_job_count: 0`, `load_to_run_ratio: 155.028327`, and `amortized_wall_ms_per_processed_request: 20577.422`. The fallback-aware gate `/mnt/nas/openclaw/reports/models/dream7b_bpu_selected_pair_cross_job_service_promotion_gate_20260610-185941/cross_job_service_promotion_gate_probe.json` records `promotion_allowed: True`, `promotion_blockers: []`, and `single_job_fallback_ok: True`. `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-185941/default_deployable_acceptance_probe.json` still records `default_deployable_status: ready_for_default_replacement_candidate_only` because `default_service_replaced: False`.

2026-06-10 default-service promotion closure: `scripts/probes/dream7b_bpu_cross_job_default_promotion_probe.sh` promotes the cross-job selected-pair route into `dream7b-bpu-batch-queue.service`, stages the long-running runtime under `/mnt/nas/openclaw/runtimes/dream7b-bpu-cross-job-default`, runs a default-queue smoke request, verifies rollback by restoring the original unit once, and then reapplies the promoted unit. `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_default_promotion_20260610-190712/cross_job_default_promotion_probe.json` records `verdict: ok_dream7b_bpu_cross_job_default_promotion_probe`, `default_service_replaced: True`, `rollback_verified: True`, and `errors: []`. `scripts/probes/dream7b_bpu_cross_job_default_service_telemetry_probe.sh` verifies the promoted default service through `/mnt/nas/openclaw/queues/dream7b-bpu`; `/mnt/nas/openclaw/reports/models/dream7b_bpu_cross_job_default_service_telemetry_20260610-191115/default_service_telemetry_probe.json` records `verdict: ok_dream7b_bpu_cross_job_default_service_telemetry_probe`, `processed_request_count: 192`, `failed_job_count: 0`, `queue_done_count: 12`, `queue_failed_count: 0`, `load_to_run_ratio: 8.734653`, `avg_bpu_loading: 9.915`, `max_bpu_loading: 98.0`, and `amortized_wall_ms_per_processed_request: 1441.545`. Replacement-time deployment acceptance `/mnt/nas/openclaw/reports/models/dream7b_bpu_deployment_acceptance_20260610-192303/deployment_acceptance_probe.json` records `verdict: ok_dream7b_bpu_deployment_acceptance_probe`, `check_count: 30`, `passed_check_count: 30`, and `errors: []`. The final acceptance `/mnt/nas/openclaw/reports/models/dream7b_bpu_default_deployable_acceptance_20260610-192303/default_deployable_acceptance_probe.json` records `default_deployable_ready: True`, `default_deployable_status: ready`, `cross_job_default_service.performance_pass_count: 3`, `cross_job_service_candidate.default_replaced_and_ready: True`, `cross_job_service_candidate.rollback_verified: True`, `blockers: []`, and `warnings: []`.
