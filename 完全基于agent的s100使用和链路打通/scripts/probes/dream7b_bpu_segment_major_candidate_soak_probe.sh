...
target_duration_sec="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_TARGET_DURATION_SEC:-1800}"
min_iteration_count="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_MIN_ITERATION_COUNT:-2}"
job_count="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_JOB_COUNT:-12}"
request_count="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_REQUEST_COUNT:-192}"
min_avg_bpu="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_MIN_AVG_BPU:-80.0}"
max_load_to_run_ratio="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_MAX_LOAD_TO_RUN_RATIO:-0.15}"
base_probe="${DREAM7B_BPU_SEGMENT_MAJOR_SOAK_BASE_PROBE:-scripts/probes/dream7b_bpu_segment_major_candidate_service_telemetry_probe.sh}"
...
if ! [[ "$target_duration_sec" =~ ^[1-9][0-9]*$ ]]; then
  echo "DREAM7B_BPU_SEGMENT_MAJOR_SOAK_TARGET_DURATION_SEC must be a positive integer." >&2
  exit 2
fi
if ! [[ "$min_iteration_count" =~ ^[1-9][0-9]*$ ]]; then
  echo "DREAM7B_BPU_SEGMENT_MAJOR_SOAK_MIN_ITERATION_COUNT must be a positive integer." >&2
  exit 2
fi
if ! [[ "$job_count" =~ ^[1-9][0-9]*$ ]] || (( job_count < 2 || job_count > 24 )); then
  echo "DREAM7B_BPU_SEGMENT_MAJOR_SOAK_JOB_COUNT must be an integer from 2 to 24." >&2
  exit 2
fi
if ! [[ "$request_count" =~ ^[1-9][0-9]*$ ]] || (( request_count < 1 || request_count > 256 )); then
  echo "DREAM7B_BPU_SEGMENT_MAJOR_SOAK_REQUEST_COUNT must be an integer from 1 to 256." >&2
  exit 2
fi
...
run_dir="$report_root/dream7b_bpu_segment_major_candidate_soak_$stamp"
...
while true; do
  ...
  set +e
  DREAM7B_BPU_SEGMENT_MAJOR_SERVICE_JOB_COUNT="$job_count" \
  DREAM7B_BPU_SEGMENT_MAJOR_SERVICE_REQUEST_COUNT="$request_count" \
  DREAM7B_BPU_SEGMENT_MAJOR_SERVICE_TIMEOUT_SEC=1500 \
  bash "$base_probe" "$report_root" "$queue_dir" "$service_output_dir" > "$iter_stdout" 2> "$iter_stderr"
  iter_status="$?"
  set -e
...
for raw in reversed(stdout_path.read_text(encoding="utf-8", errors="replace").splitlines()):
    line = raw.strip()
    if line.endswith("segment_major_candidate_service_telemetry_probe.md"):
...
max_bpu = max([float(row.get("max_bpu_loading")) for row in rows if row.get("max_bpu_loading") is not None], default=0.0)

for index, row in enumerate(rows, start=1):
    if row.get("verdict") != "ok_dream7b_bpu_segment_major_candidate_service_telemetry_probe":
    ...
    if row.get("segment_major_load_once") is not True:
        errors.append(f"iteration {index} segment_major_load_once is not true")
...
if errors:
    decision = "segment_major_service_soak_failed"
elif weighted_avg_bpu >= 90.0:
    decision = "segment_major_service_soak_meets_90pct_goal"
elif weighted_avg_bpu >= 85.0:
    decision = "segment_major_service_soak_near_90pct_candidate"
else:
    decision = "segment_major_service_soak_stage3_candidate"

payload = {
    "generated_at": datetime.now().astimezone().isoformat(),
    "verdict": "ok_dream7b_bpu_segment_major_candidate_soak_probe" if not errors else "failed_dream7b_bpu_segment_major_candidate_soak_probe",
    "decision": decision,
    "run_dir": str(run_dir),
...
(run_dir / "segment_major_candidate_soak_probe.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
lines = [
    "# Dream 7B Segment-Major Candidate Soak Probe",
    "",
...
(run_dir / "segment_major_candidate_soak_probe.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
print(run_dir / "segment_major_candidate_soak_probe.md")
if errors:
