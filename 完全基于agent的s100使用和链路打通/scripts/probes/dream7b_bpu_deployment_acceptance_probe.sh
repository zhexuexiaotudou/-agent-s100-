...
def latest_json_where(pattern, predicate):
    paths = [Path(item) for item in glob.glob(str(report_root / pattern))]
    paths = [item for item in paths if item.is_file()]
...
latest_text_queue_run_path, latest_text_queue_run = latest_json("dream7b_bpu_text_queue_run_*/text_queue_run.json")
text_queue_run_path, text_queue_run = latest_json_where(
    "dream7b_bpu_text_queue_run_*/text_queue_run.json",
    lambda data: data.get("verdict") == "ok_dream7b_bpu_text_queue_run",
)
if (
    latest_text_queue_run_path
    and text_queue_run_path
    and latest_text_queue_run_path != text_queue_run_path
):
    warnings.append(
        "latest legacy text_queue_run is not OK under the promoted cross-job default service; "
        f"using most recent OK legacy text_queue_run: {text_queue_run_path}"
    )
if text_queue_run is None:
    add_check(
        ...
        False,
        {
            "reason": "missing ok text_queue_run.json",
            "latest_verdict": (latest_text_queue_run or {}).get("verdict") if latest_text_queue_run else None,
            "latest_errors": (latest_text_queue_run or {}).get("errors") if latest_text_queue_run else None,
        },
    )
else:
...
if utilization_gap is None:
    ...
    selected_pair_candidate_service_batch_counts = selected_pair_candidate_service_telemetry.get("batch_counts")
    selected_pair_candidate_service_batch_counts_valid = (
        isinstance(selected_pair_candidate_service_batch_counts, list)
        and len(selected_pair_candidate_service_batch_counts) >= min_batch_generate_sustained_round_count
        and all(int(item or 0) == min_batch_capacity for item in selected_pair_candidate_service_batch_counts)
    )
    ...
    resplit_segment_plan = resplit_batch_telemetry.get("segment_plan") or resplit_window_cost.get("segment_plan")
    if resplit_segment_plan == "phase1-topload-adjacent":
        resplit_expected_segments_per_forward = 20
        resplit_expected_window_count = 10
    elif resplit_segment_plan == "resplit-topwindow-adjacent":
        resplit_expected_segments_per_forward = 18
        resplit_expected_window_count = 9
    else:
        resplit_expected_segments_per_forward = 14
        resplit_expected_window_count = 7
    ...
    ok = (
        ...
        and selected_pair_candidate_service_batch_counts_valid
        ...
        and selected_pair_cross_job_comparison.get("cross_job_wall_time_improved") in (True, False)
        and int(resplit_batch_telemetry.get("batch_count") or 0) >= min_batch_capacity
        and resplit_batch_telemetry.get("segment_plan") in ("resplit-adjacent", "resplit-topwindow-adjacent", "phase1-topload-adjacent")
        ...
        and resplit_batch_telemetry.get("segment_event_count") == min_batch_capacity * resplit_expected_segments_per_forward
        ...
        and resplit_window_cost.get("segment_plan") == resplit_segment_plan
        ...
        and resplit_window_cost.get("segment_event_count") == min_batch_capacity * resplit_expected_segments_per_forward
        and resplit_window_cost.get("window_count") == resplit_expected_window_count
    ...
    add_check(
        ...
        ok,
        {
            ...
            "selected_pair_candidate_service_batch_counts": selected_pair_candidate_service_batch_counts,
            "selected_pair_candidate_service_batch_counts_valid": selected_pair_candidate_service_batch_counts_valid,
...
if selected_pair_candidate_service_telemetry is None:
    ...
    batch_counts_valid = (
        isinstance(batch_counts, list)
        and len(batch_counts) >= min_batch_generate_sustained_round_count
        and all(int(item or 0) == min_batch_capacity for item in batch_counts)
    )
    ok = (
        ...
        and selected_pair_candidate_service_telemetry.get("expected_window_execution_mode") == "selected-pair-resident"
        and selected_pair_candidate_service_telemetry.get("expected_child_process_count") == 2
        and int(selected_pair_candidate_service_telemetry.get("job_count") or 0) >= min_batch_generate_sustained_round_count
        and selected_pair_candidate_service_telemetry.get("request_count") == min_batch_capacity
        and int(selected_pair_candidate_service_telemetry.get("processed_request_count") or 0) >= min_systemd_telemetry_requests
        and int(selected_pair_candidate_service_telemetry.get("accepted_request_count") or 0) >= min_systemd_telemetry_requests
        and selected_pair_candidate_service_telemetry.get("deferred_request_count") == 0
        and int(selected_pair_candidate_service_telemetry.get("result_count") or 0) >= min_systemd_telemetry_requests
        and batch_counts_valid
    ...
    add_check(
        ...
        ok,
        {
            ...
            "batch_counts_valid": batch_counts_valid,
...
if selected_pair_cross_job_reuse is None:
    ...
    candidate_batch_counts = candidate_metrics.get("batch_counts")
    candidate_batch_counts_valid = (
        isinstance(candidate_batch_counts, list)
        and len(candidate_batch_counts) >= min_batch_generate_sustained_round_count
        and all(int(item or 0) == min_batch_capacity for item in candidate_batch_counts)
    )
    ok = (
        ...
        and int(candidate_metrics.get("processed_request_count") or 0) >= min_systemd_telemetry_requests
        and candidate_batch_counts_valid
        ...
        and comparison.get("cross_job_wall_time_improved") in (True, False)
    ...
    add_check(
        ...
        ok,
        {
            ...
            "candidate_batch_counts_valid": candidate_batch_counts_valid,
