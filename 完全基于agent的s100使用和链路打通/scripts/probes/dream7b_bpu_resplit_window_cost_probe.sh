...
if [[ -n "${DREAM7B_BPU_RESPLIT_WINDOW_COST_EXPECTED_WINDOW_COUNT:-}" ]]; then
  expected_window_count="${DREAM7B_BPU_RESPLIT_WINDOW_COST_EXPECTED_WINDOW_COUNT}"
elif [[ "$expected_segment_plan" == "resplit-topwindow-adjacent" ]]; then
  expected_window_count="9"
elif [[ "$expected_segment_plan" == "phase1-topload-adjacent" ]]; then
  expected_window_count="10"
else
  expected_window_count="7"
fi
if [[ -n "${DREAM7B_BPU_RESPLIT_WINDOW_COST_EXPECTED_SEGMENT_EVENT_COUNT:-}" ]]; then
  expected_segment_event_count="${DREAM7B_BPU_RESPLIT_WINDOW_COST_EXPECTED_SEGMENT_EVENT_COUNT}"
elif [[ "$expected_segment_plan" == "resplit-topwindow-adjacent" ]]; then
  expected_segment_event_count="288"
elif [[ "$expected_segment_plan" == "phase1-topload-adjacent" ]]; then
  expected_segment_event_count="320"
else
...
case "$expected_segment_plan" in
  ...
  *)
    echo "DREAM7B_BPU_RESPLIT_WINDOW_COST_EXPECTED_SEGMENT_PLAN must be resplit-adjacent, resplit-topwindow-adjacent, or phase1-topload-adjacent." >&2
    exit 2
    ;;
esac
