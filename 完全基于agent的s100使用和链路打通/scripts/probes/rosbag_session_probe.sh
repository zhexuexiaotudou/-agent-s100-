...
duration="${ROSBAG_SESSION_SELFTEST_SECONDS:-4}"
...
case "$duration" in
  ''|*[!0-9]*) echo "ROSBAG_SESSION_SELFTEST_SECONDS must be an integer" >&2; exit 2 ;;
esac

if (( duration < 2 || duration > 20 )); then
  echo "ROSBAG_SESSION_SELFTEST_SECONDS must be between 2 and 20" >&2
  exit 2
fi
...
session_id="rosbag_session_$timestamp"
bag_dir="$dataset_root/$session_id"
report="$report_dir/${session_id}.md"
state_dir="$report_dir/rosbag_sessions"
record_log="$report_dir/${session_id}.record.log"
bag_info="$report_dir/${session_id}.bag_info.txt"
ros_log_dir="$report_dir/${session_id}.ros_logs"
...
if [[ -d "$bag_dir" ]]; then
  {
    ...
    echo "- capture_type: rosbag_start_stop_selftest"
    ...
    echo
    echo "- This self-test starts and stops a bounded recorder automatically."
    echo "- No robot motion command is sent."
    echo "- Only low-risk ROS status topics are recorded."
    echo
    echo "## Files"
...
{
  echo "# ROS Bag Session Self-Test"
  echo
  echo "- generated_at: $(date -Is)"
  echo "- session_id: $session_id"
  ...
  echo
  echo '```text'
  sed -n '1,80p' "$state_file" 2>/dev/null || true
  echo '```'
  echo
  echo "## Bag Files"
  echo
  ...
  echo
  echo '```text'
  sed -n '1,160p' "$record_log" 2>/dev/null || true
  echo '```'
  if [[ -f "$bag_info" ]]; then
    echo
    echo "## Bag Info"
