...
diffuse_cli_available="$(cmd_status diffuse-cli)"
dream7b_cli_available="$(cmd_status dream7b-cli)"
dream7b_text_available="$(cmd_status dream7b-text)"
...
nas_model_dir="/mnt/nas/openclaw/models"
nas_fstype="$(findmnt -rn -o FSTYPE --target /mnt/nas/openclaw 2>/dev/null | head -1 || true)"
case "$nas_fstype" in
  nfs|nfs4|cifs|smb3)
    model_dirs=("$nas_model_dir" "${model_dirs[@]}")
    ;;
  *)
    model_dir_lines="$nas_model_dir: skipped_not_mounted"
    if [[ -n "$nas_fstype" ]]; then
      model_dir_lines+=" (current fstype: $nas_fstype)"
    fi
    model_dir_lines+=$'\n'
    ;;
esac
...
model_file_count=0
dream_named_count=0
model_dir_lines="${model_dir_lines:-}"
model_file_lines=""
...
[[ "$diffuse_cli_available" == "yes" || "$dream7b_cli_available" == "yes" || "$dream7b_text_available" == "yes" || -x "/mnt/nas/openclaw/runtimes/diffuse-cpp/build/diffuse-cli" ]] && runtime_candidates+=("diffuse-cpp")
...
python3 - "$json" \
  ...
  "$diffuse_cli_available" "$dream7b_cli_available" "$dream7b_text_available" \
...
payload = {
    ...
    "commands": {
        ...
        "diffuse-cli": diffuse_cli_available == "yes",
        "dream7b-cli": dream7b_cli_available == "yes",
        "dream7b-text": dream7b_text_available == "yes",
...

{
  ...
  echo "| diffuse-cli | $diffuse_cli_available |"
  echo "| dream7b-cli | $dream7b_cli_available |"
  echo "| dream7b-text | $dream7b_text_available |"
