...
def valid_sustained_batch_counts(batch_counts):
    if not isinstance(batch_counts, list):
        return False
    if len(batch_counts) < min_sustained_round_count:
        return False
    return all(int(item or 0) == min_batch_count for item in batch_counts)
...
selected_pair_candidate_service_batch_counts = (selected_pair_candidate_service_telemetry or {}).get("batch_counts")
selected_pair_candidate_service_round_count = len(selected_pair_candidate_service_batch_counts) if isinstance(selected_pair_candidate_service_batch_counts, list) else 0
selected_pair_candidate_service_batch_counts_valid = valid_sustained_batch_counts(selected_pair_candidate_service_batch_counts)
if isinstance(selected_pair_candidate_service_telemetry, dict):
    ...
    if not selected_pair_candidate_service_batch_counts_valid:
        errors.append(
            "unexpected selected-pair candidate service batch_counts: "
            f"{selected_pair_candidate_service_batch_counts}; expected at least "
            f"{min_sustained_round_count} rounds of batch_count={min_batch_count}"
        )
...
resplit_segment_plan = resplit_batch_forward_metrics.get("segment_plan") or (resplit_window_cost or {}).get("segment_plan")
...
if isinstance(resplit_batch_telemetry, dict):
    ...
    if resplit_batch_forward_metrics.get("segment_plan") not in ("resplit-adjacent", "resplit-topwindow-adjacent", "phase1-topload-adjacent"):
    ...
    if resplit_batch_forward_metrics.get("segment_event_count") != min_batch_count * resplit_expected_segments_per_forward:
...
if isinstance(resplit_window_cost, dict):
    ...
    if resplit_window_cost.get("segment_plan") != resplit_segment_plan:
    ...
    if resplit_window_cost.get("segment_event_count") != min_batch_count * resplit_expected_segments_per_forward:
        errors.append(f"unexpected resplit window cost segment_event_count: {resplit_window_cost.get('segment_event_count')}")
    if resplit_window_cost.get("window_count") != resplit_expected_window_count:
        errors.append(f"unexpected resplit window cost window_count: {resplit_window_cost.get('window_count')}")
    if len(resplit_window_cost.get("ranked_by_load") or []) != resplit_expected_window_count:
...
payload = {
    ...
    "selected_pair_candidate_service_telemetry": {
        ...
        "batch_counts": selected_pair_candidate_service_batch_counts,
        "round_count": selected_pair_candidate_service_round_count,
        "batch_counts_valid": selected_pair_candidate_service_batch_counts_valid,
