...
if summary_path.is_file():
    ...
    fallback_summaries = sorted(
        Path(output_dir).glob("runs/*/segment_major_queue_summary.json"),
        key=lambda item: item.stat().st_mtime,
        reverse=True,
    )
    for candidate_path in fallback_summaries:
        try:
            candidate_summary = json.loads(candidate_path.read_text(encoding="utf-8"))
        except Exception:
            continue
        durable = candidate_summary.get("durable_state") or {}
...
if isinstance(summary, dict):
    ...
    if runner_verdict == "ok_dream7b_bpu_batch_queue_runner":
    ...
    elif runner_verdict == "ok_dream7b_bpu_segment_major_load_once_queue_runner":
        processed_count = summary.get("processed_request_count")
        accepted_count = summary.get("processed_request_count")
        deferred_count = 0
        skipped_count = summary.get("skipped_request_count")
        batch_run_count = summary.get("processed_job_count")
        if processed_count != 1:
            errors.append(f"unexpected processed_request_count: {processed_count}")
        if accepted_count != 1:
            errors.append(f"unexpected accepted_count: {accepted_count}")
        if skipped_count != 0:
            errors.append(f"unexpected skipped_request_count: {skipped_count}")
        if int(summary.get("failed_job_count") or 0) != 0:
            errors.append(f"unexpected failed_job_count: {summary.get('failed_job_count')}")
        if batch_run_count != 1:
            errors.append(f"unexpected processed_job_count: {batch_run_count}")
    else:
        errors.append(f"unexpected runner verdict: {runner_verdict}")
    ...
    if runner_verdict == "ok_dream7b_bpu_batch_queue_runner":
        ...
        batch_count = summary.get("processed_request_count")
        execution_mode = "segment_major_load_once"
        window_execution_mode = "segment-major"
        child_process_count = 0
        durable = summary.get("durable_state") or {}
        results_path = Path(durable.get("results_jsonl") or "")
    ...
    if runner_verdict == "ok_dream7b_bpu_batch_queue_runner" and batch_count != 1:
        errors.append(f"unexpected batch_count: {batch_count}")
    if runner_verdict == "ok_dream7b_bpu_batch_queue_runner" and execution_mode != "pair_window_batch":
        errors.append(f"unexpected execution_mode: {execution_mode}")
    if runner_verdict == "ok_dream7b_bpu_batch_queue_runner" and window_execution_mode != "window-batch":
    ...
    if runner_verdict == "ok_dream7b_bpu_batch_queue_runner":
        ...
        amortized_wall_ms = float(summary.get("amortized_wall_ms_per_processed_request") or 0.0)
