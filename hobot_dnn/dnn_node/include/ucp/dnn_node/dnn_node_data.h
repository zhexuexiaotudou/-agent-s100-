...
struct DnnNodePara {
  ...
  // - HB_UCP_BPU_CORE_ANY: 不指定BPU核，推理任务交替使用两个BPU核，实现负载均衡
  // - HB_UCP_BPU_CORE_0: 推理任务使用BPU 0
  // - HB_UCP_BPU_CORE_1: 推理任务使用BPU 1
  // 为task指定使用的BPU核，bpu_core_ids为空或者size等于task_num
  // 为空不指定，即使用BPU_CORE_ANY模式
  // 当bpu_core_ids size等于task_num时，分别为每个task指定对应的BPU核
  // 例如task_num为2，bpu_core_ids为{HB_UCP_BPU_CORE_0, HB_UCP_BPU_CORE_1}时，两个task分别运行在BPU 0 和 BPU 1 上
