...
int DnnNodeImpl::TaskInit() {
  ...
  // 2. 创建idle running task
  {
    ...
    auto bpu_core_id = HB_UCP_BPU_CORE_0;
    ...
    if (static_cast<int>(dnn_node_para_ptr_->bpu_core_ids.size()) == task_num) {
      ...
      for (int idx = 0; idx < task_num; idx++) {
        ...
        if (dnn_node_para_ptr_->bpu_core_ids.at(idx) !=
          ...
          if (HB_UCP_BPU_CORE_0 == bpu_core_id) {
            bpu_core_id = HB_UCP_BPU_CORE_1;
          } else if (HB_UCP_BPU_CORE_1 == bpu_core_id) {
            bpu_core_id = HB_UCP_BPU_CORE_0;
          } else {
            bpu_core_id = HB_UCP_BPU_CORE_0;
          }
        }
...
TaskId DnnNodeImpl::AllocTask(int timeout_ms) {
  ...
  TaskId task_id = -1;
  int32_t bpu_core_id = HB_UCP_BPU_CORE_0;
  if (!dnn_rt_para_) {
    return task_id;
  }
  ...
  auto alloc_task = [this, &task_id, &bpu_core_id]() {
    ...
    bpu_core_id = idle_task->second->backend;
  ...
  if (en_set_task_para_) {
    ...
    hbUCPSchedParam ctrl_param;
    HB_UCP_INITIALIZE_SCHED_PARAM(&ctrl_param);
    ctrl_param.backend = bpu_core_id;
    RCLCPP_INFO(rclcpp::get_logger("dnn"),
...
int DnnNodeImpl::ReleaseTask(const TaskId &task_id) {
  ...
  // 上一次推理任务使用的BPU核
  auto last_bpu_core_id = dnn_rt_para_->running_tasks[task_id]->backend;
  // 本次推理任务使用的BPU核
  ...
  if (static_cast<int>(dnn_node_para_ptr_->bpu_core_ids.size()) ==
    ...
    // 交替使用BPU核
    if (HB_UCP_BPU_CORE_0 == last_bpu_core_id) {
      present_bpu_core_id = HB_UCP_BPU_CORE_1;
    } else if (HB_UCP_BPU_CORE_1 == last_bpu_core_id) {
      present_bpu_core_id = HB_UCP_BPU_CORE_0;
    } else {
      present_bpu_core_id = HB_UCP_BPU_CORE_0;
    }
  }
...
int DnnNodeImpl::GetModelInputSize(int32_t input_index, int &w, int &h) {
  ...
  if (properties.quantizeAxis == HB_DNN_LAYOUT_NHWC) {
    w = properties.validShape.dimensionSize[2];
    h = properties.validShape.dimensionSize[1];
  } else if (properties.quantizeAxis == HB_DNN_LAYOUT_NCHW) {
