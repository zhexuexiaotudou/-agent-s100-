...
int LoadConfig(const rapidjson::Document &document){
  ...
  if (document.HasMember("output_order")) {
    ...
    if(InitOutputOrder(yolo3_config_.output_order, model_output_count) < 0){
      return -1;
    }
  } else {
    for (int i = 0; i < model_output_count; i++) {
      yolo3_config_.output_order.push_back(i);
    }
  }
  return 0;
}
...
int PostProcess(std::vector<std::shared_ptr<DNNTensor>> &tensors,
  ...
  #ifdef BPU_LIBDNN
    auto tensorlayout = tensors[i]->properties.tensorLayout;
  #endif
  #ifdef BPU_UCP
    auto tensorlayout = tensors[i]->properties.quantizeAxis;
  #endif
...
void PostProcessNHWC(std::shared_ptr<DNNTensor> tensor,
  ...
  float *data = tensor->GetTensorData<float>();
...
void PostProcessNCHW(std::shared_ptr<DNNTensor> tensor,
  ...
  int aligned_h = tensor->properties.stride[1] / tensor->properties.stride[2];
  int aligned_w = tensor->properties.stride[2] / tensor->properties.stride[3];
  int channel_aligned = tensor->properties.stride[0] / tensor->properties.stride[1];
...
void PostProcessQuantiScaleNHWC(std::shared_ptr<DNNTensor> tensor,
                                ...
                                std::vector<Detection> &dets) {
  tensor->CACHE_INVALIDATE();
  int32_t *data = tensor->GetTensorData<int32_t>();
