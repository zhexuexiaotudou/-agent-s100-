...
def extract_zip_normalized(zip_path: Path, target: Path) -> None:
...
def latest_stage1_package(root: Path) -> Path | None:
    patterns = [
        root / "stage1_input",
        root / "evidence_for_gptpro",
        root,
    ]
    candidates: list[Path] = []
    for directory in patterns:
        if directory.exists():
            candidates.extend(directory.glob("ai_nas_harness_stage1*_gptpro_*.zip"))
            candidates.extend(directory.glob("ai_nas_harness_stage1_fixed_gptpro_*.zip"))
    candidates = sorted(set(candidates), key=lambda p: p.stat().st_mtime)
    return candidates[-1] if candidates else None


def stage1_package_or_die(root: Path) -> Path:
    package = latest_stage1_package(root)
    if package:
        return package
    package_root = find_package_root(root)
    package = latest_stage1_package(package_root)
    if package:
        return package
    raise FileNotFoundError(f"stage1 package not found under {root} or {package_root}")
...
def package_reproducibility(report_root: Path, package_zip: Path | None = None) -> dict[str, Any]:
    ...
    extract_zip_normalized(package, temp)
    ...
    if missing_test.exists():
        ...
        old_env = {key: os.environ.get(key) for key in ["AI_NAS_PACKAGE_ROOT", "AI_NAS_REPO_ROOT", "AI_NAS_PRODUCTION_CONTEXT_ROOT"]}
        try:
            os.environ["AI_NAS_PACKAGE_ROOT"] = str(temp)
            os.environ["AI_NAS_REPO_ROOT"] = str(temp)
            os.environ["AI_NAS_PRODUCTION_CONTEXT_ROOT"] = str(temp / "production_context")
...
def existing_gate_hard_fail(report_root: Path) -> dict[str, Any]:
    ...
    extracted_context = ROOT / "tmp" / "stage1_package_repro_check" / "production_context"
    if (extracted_context / "scripts" / "probes" / "ai_nas_allowlisted_tool.sh").exists():
        source_context = extracted_context
    else:
        source_context = find_production_context_root(ROOT)
    if source_context.resolve() == ROOT.resolve():
        add_check(checks, failures, "negative test production context isolated", False, "resolver returned repo root; refusing to copy full repo")
        return gate_payload("existing_gate_hard_fail_test", checks, failures, {"source_context": str(source_context)})
    shutil.copytree(ROOT / "config", bad_root / "config", dirs_exist_ok=True)
    (bad_root / "ai_nas_harness").mkdir(parents=True, exist_ok=True)
    shutil.copytree(source_context, bad_root / "production_context", dirs_exist_ok=True)
    ...
    bad = subprocess.run(
        ...
        env={
            **os.environ,
            "AI_NAS_REPO_ROOT": str(bad_root),
            "AI_NAS_PACKAGE_ROOT": str(bad_root),
            "AI_NAS_PRODUCTION_CONTEXT_ROOT": str(bad_root / "production_context"),
        },
    )
...
def cloud_redaction_hardening(report_root: Path) -> dict[str, Any]:
    cases = [
        ...
        "/home/sunrise/Personal/Family/id_card_photo.jpg",
        "C:\\Users\\zhexu\\Documents\\Private\\contract.docx",
        "../Personal/Inbox/denied.txt",
        "denied_acl_snippet: Personal/Photos/family_face.jpg",
        "summarize /mnt/nas/Personal/Photos/family_photo.jpg without leaking privacy",
    ]
    checks: list[dict[str, Any]] = []
    failures: list[str] = []
    results = []
...
def sidecar_mock_isolation(report_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        add_check(checks, failures, "mock read-only tools only", set(exposed) == {"mock.nas_search", "mock.document_rag"}, exposed)
    finally:
        proc.terminate()
        try:
...
def rollback_gate(report_root: Path) -> dict[str, Any]:
    ...
    bash = str(git_bash) if git_bash.exists() else (shutil.which("bash") or shutil.which("bash.exe"))
    if not bash:
        add_check(checks, failures, "bash available for stop command", False)
        completed = subprocess.CompletedProcess([str(stop_script)], 127, "", "bash not found")
    else:
        add_check(checks, failures, "bash available for stop command", True, bash)
        completed = subprocess.run([bash, str(stop_script)], cwd=ROOT, text=True, capture_output=True, timeout=30)
