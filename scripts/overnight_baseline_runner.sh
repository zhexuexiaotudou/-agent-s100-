...
latest_acceptance="$(find "$nas_root/reports/baseline-status" -type f -name 'baseline_acceptance_*.md' -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1 {$1=""; sub(/^ /, ""); print}')"
...
log_event "info" "runner_finish" "ok" "iterations=$iteration latest_stability=${latest_stability:-missing} latest_baseline=${latest_baseline:-missing} latest_gap=${latest_gap:-missing} latest_acceptance=${latest_acceptance:-missing} latest_teacher=${latest_teacher:-missing} latest_convergence=${latest_convergence:-missing}"
