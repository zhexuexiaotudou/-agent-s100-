...
request_count="${DREAM7B_BPU_SYSTEMD_CANARY_REQUEST_COUNT:-1}"
timeout_sec="${DREAM7B_BPU_SYSTEMD_CANARY_TIMEOUT_SEC:-180}"
poll_interval_sec="${DREAM7B_BPU_SYSTEMD_CANARY_POLL_INTERVAL_SEC:-2}"
...
if ! [[ "$request_count" =~ ^[0-9]+$ ]] || (( request_count < 1 || request_count > 4 )); then
  echo "DREAM7B_BPU_SYSTEMD_CANARY_REQUEST_COUNT must be an integer from 1 to 4." >&2
  exit 2
fi
if ! [[ "$timeout_sec" =~ ^[0-9]+$ ]] || (( timeout_sec < 1 )); then
  echo "DREAM7B_BPU_SYSTEMD_CANARY_TIMEOUT_SEC must be a positive integer." >&2
  exit 2
fi
if ! [[ "$poll_interval_sec" =~ ^[0-9]+$ ]] || (( poll_interval_sec < 1 )); then
  echo "DREAM7B_BPU_SYSTEMD_CANARY_POLL_INTERVAL_SEC must be a positive integer." >&2
  exit 2
fi
...
run_dir="$report_root/dream7b_bpu_batch_queue_systemd_canary_$stamp"
mkdir -p "$run_dir"
job_name="systemd_canary_${stamp}.jsonl"
...
if [[ "$service_status_before" != "active" ]]; then
  echo "Service is not active before canary probe: $service_status_before" >&2
  exit 3
fi
...
for index in range(request_count):
    ...
    rows.append(
        {
            "request_id": f"systemd-canary-{stamp}-{index + 1:03d}",
...
payload = {
    ...
    "verdict": "ok_dream7b_bpu_batch_queue_systemd_canary_probe" if not errors else "failed_dream7b_bpu_batch_queue_systemd_canary_probe",
...
(run_dir / "systemd_canary_probe.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
error_lines = [f"- {item}" for item in errors] if errors else ["- none"]
(run_dir / "systemd_canary_probe.md").write_text(
    "\n".join([
        "# Dream 7B BPU Batch Queue Systemd Canary Probe",
        "",
...
print(run_dir / "systemd_canary_probe.md")
if errors:
