...
run_cmd="${DREAM7B_BPU_TEXT_QUEUE_RUN_CMD:-dream7b-bpu-text-queue-run}"
...
if ! command -v "$run_cmd" >/dev/null 2>&1; then
  echo "Missing deployed command: $run_cmd" >&2
  exit 3
fi
...
run_stdout="$run_dir/text_queue_run.stdout"
run_stderr="$run_dir/text_queue_run.stderr"
...
run_cmd = sys.argv[25]
run_json = Path(sys.argv[26])
run_md = Path(sys.argv[27])
run_stdout = Path(sys.argv[28])
run_stderr = Path(sys.argv[29])
...
if run_json.is_file():
    ...
    errors.append(f"missing text_queue_run.json: {run_json}")
...
if isinstance(run_payload, dict):
    if run_payload.get("verdict") != "ok_dream7b_bpu_text_queue_run":
        errors.append(f"unexpected run verdict: {run_payload.get('verdict')}")
    if run_payload.get("job_name") != job_name:
        errors.append(f"unexpected run job_name: {run_payload.get('job_name')}")
    if run_payload.get("request_id") != request_id:
        errors.append(f"unexpected run request_id: {run_payload.get('request_id')}")
    if run_payload.get("queue_dir") != queue_dir:
        errors.append(f"unexpected run queue_dir: {run_payload.get('queue_dir')}")
    if run_payload.get("output_dir") != output_dir:
        errors.append(f"unexpected run output_dir: {run_payload.get('output_dir')}")
    if run_payload.get("summary_path") != str(summary_path):
        errors.append(f"unexpected run summary_path: {run_payload.get('summary_path')}")
    if run_payload.get("submit_cmd") != submit_cmd:
...
payload = {
    ...
    "run": run_payload,
    "run_verdict": run_payload.get("verdict") if isinstance(run_payload, dict) else None,
...
(run_dir / "text_queue_systemd_probe.md").write_text(
    "\n".join([
        ...
        f"- run_cmd: {payload['run_cmd']}",
        f"- run_verdict: {payload['run_verdict']}",
        f"- run_json: {payload['run_json']}",
        f"- run_md: {payload['run_md']}",
