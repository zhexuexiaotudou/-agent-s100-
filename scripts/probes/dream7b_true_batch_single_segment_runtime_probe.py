...
def main() -> int:
    ...
    parser.add_argument("--hbm-root", required=True)
    ...
    run_dir = report_root / f"dream7b_true_batch_single_segment_runtime_{stamp}_seg{args.segment_index:02d}_b{args.batch_size}"
    ...
    try:
        ...
        if index == 0:
            ...
            hidden = np.zeros((args.batch_size, args.seq_len, args.hidden_size), dtype=np.float32)
            inputs = {"_input_0": hidden, "_input_1": position_ids}
            input_shapes = {"_input_0": list(hidden.shape), "_input_1": list(position_ids.shape)}
    ...
    payload = {
        ...
        "verdict": "ok_dream7b_true_batch_single_segment_runtime" if not errors else "failed_dream7b_true_batch_single_segment_runtime",
    ...
    report_json = run_dir / "true_batch_single_segment_runtime.json"
    report_md = run_dir / "true_batch_single_segment_runtime.md"
    report_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=json_default) + "\n", encoding="utf-8")
    report_md.write_text(
        "\n".join(
            [
                "# Dream7B True Batch Single Segment Runtime",
                "",
                ...
                f"- output_shape: {row['output_shape'] if row else None}",
                f"- load_ms: {row['load_ms'] if row else None}",
                f"- run_ms: {row['run_ms'] if row else None}",
                "",
                "## Errors",
                "",
