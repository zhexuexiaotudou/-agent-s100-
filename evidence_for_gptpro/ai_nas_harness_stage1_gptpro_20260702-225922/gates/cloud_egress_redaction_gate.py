...
def _contains_term(text: str, term: str) -> bool:
    return re.search(re.escape(term), text, flags=re.I) is not None
...
def run_gate(report_root: str | Path | None = None) -> dict[str, Any]:
    ...
    for item in scenarios:
        ...
        if scenario_id == "web_cloud_research_redacted":
            ...
            leaked = [term for term in terms if _contains_term(preview, str(term))]
            ...
            check(checks, failures, "web cloud egress contains no configured private terms", not leaked, leaked)
            check(checks, failures, "web cloud egress uses configured replacement", replacement in preview, preview)
        else:
