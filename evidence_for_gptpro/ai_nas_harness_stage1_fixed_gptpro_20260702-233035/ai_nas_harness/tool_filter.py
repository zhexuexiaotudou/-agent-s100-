...
from .argument_policy import ArgumentPolicyFilter
from .path_resolver import resolve_asset
...
class ToolExposureFilter:
    def __init__(
        ...
        *,
        ...
        arg_policy_path: str | Path | None = None,
    ):
        ...
        self.dispatcher_path = resolve_asset(
            self.policy["dispatcher"]["local_path"],
            repo_root=repo_root(),
        ...
        default_arg_policy = repo_root() / "config" / "workspace_arg_policy.yaml"
        self.arg_filter = ArgumentPolicyFilter(arg_policy_path or default_arg_policy) if arg_policy_path or default_arg_policy.exists() else None
    ...
    def call_tool(
        ...
        *,
        ...
        arg_decision = self.arg_filter.validate(workspace_id, tool_id, args) if self.arg_filter else {"allowed": True, "trace_args": args}
        if not arg_decision.get("allowed"):
            return self._deny(run_id, workspace_id, tool_id, str(arg_decision.get("reason_code") or "argument_policy_denied"), arg_decision.get("trace_args") or [])
        if tool_id in self.approval_required_tools(workspace_id) and not approval_token:
            trace_args = arg_decision.get("trace_args") if isinstance(arg_decision, dict) else args
            return self._deny(run_id, workspace_id, tool_id, "approval_required", trace_args if isinstance(trace_args, list) else [trace_args])
        if dry_run:
            result = {
                ...
                "args_hash": arg_decision.get("args_hash"),
                "arg_policy": arg_decision.get("policy"),
            }
            if self.trace_writer and run_id:
                self.trace_writer.add_tool_call(run_id, workspace_id, tool_id, "allowed_dry_run", args=[arg_decision.get("trace_args")], result=result)
            return result
