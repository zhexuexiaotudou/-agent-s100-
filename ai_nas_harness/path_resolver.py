...
def find_package_root(start: str | Path | None = None) -> Path:
    env_root = os.environ.get("AI_NAS_PACKAGE_ROOT")
    ...
    for candidate in candidates:
        ...
        for parent in [current, *current.parents]:
            if _has_repo_markers(parent) or (parent / "stage1_input").exists():
                return parent
...
def find_repo_root(start: str | Path | None = None) -> Path:
    ...
    package_root = os.environ.get("AI_NAS_PACKAGE_ROOT")
    candidates: list[Path] = []
    if env_root:
        candidates.append(Path(env_root))
    if package_root:
...
def find_production_context_root(repo_root: str | Path | None = None, explicit: str | Path | None = None) -> Path:
    ...
    if package_root:
        ...
        candidates.extend([pkg / "production_context", pkg / "tmp" / "stage1_input_extracted" / "production_context"])
    candidates.extend([root / "production_context", root / "tmp" / "stage1_input_extracted" / "production_context", root])
    for candidate in candidates:
