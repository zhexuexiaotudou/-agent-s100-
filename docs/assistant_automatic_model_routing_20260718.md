...
| 公开、无隐私、无本地工具意图，且明确要求联网检索最新外部信息的任务；或具有同等时效性需求的复杂任务 | `web_cloud_research` | `custom-gateway/MiniMax-M2.7` | OpenClaw loopback bridge `127.0.0.1:18082` |
...
MiniMax 不是“复杂任务的默认大模型”。策略必须确认 `privacy_level=0`、需要公开网络并具有明确
时效性（例如当前、最新、近期、价格、固件、漏洞或新闻），并且任务复杂度不低于 2 或用户明确
要求联网检索，才允许外发。公开但不要求最新信息的复杂推理仍交给本地 7B；用户明确要求离线时
也保持本地。

明确的公开联网命令（例如“联网搜索最新 AI 新闻”）不需要用户为了越过复杂度门槛而补长提示词；
`explicit_web_search=true` 可替代复杂度下限，但不能绕过隐私、本地工具、本地数据、禁止联网和
时效性检查。该字段会写入回答详情，便于区分“明确联网短句”和普通简单对话。
