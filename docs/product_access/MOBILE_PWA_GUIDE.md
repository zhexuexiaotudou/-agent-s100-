...
2026-07-17 已直接对 S100P LAN 入口执行浏览器验收：360x800、390x844、430x932、768x1024 均无页面级横向溢出，console error 为 0；证据位于 `reports/access/raw/`。Tailscale HTTPS 入口随后实测返回 `/ui`、manifest 和 service worker，但这仍不等同于物理手机扫码或浏览器安装确认。物理手机与 PWA 安装仍待用户终端验证，LAN HTTP 不伪装成 secure context。
