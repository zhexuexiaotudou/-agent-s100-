...
- 相册整块显示“读取失败 / Failed to fetch”，同时 `journalctl -u digua-product-access.service` 出现 `sqlite3.OperationalError: database is locked`：先确认部署至少包含 PR #53。修复后的已有桥接会话不会为每个缩略图重复访问 NAS 身份库；新桥接遇到持续锁竞争会返回 `503 upstream_identity_bridge_unavailable` JSON。若浏览器仍报告原生 `Failed to fetch` 且日志没有 HTTP 状态码，说明请求处理线程仍在异常退出。不要重置 identity DB；检查产品访问与门户健康、NAS mount 和并发写入者，锁释放后重新请求。
