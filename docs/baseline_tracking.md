...
| B-003 | 图片 caption / 语义搜索 baseline | doing | NAS-backed metadata caption 和 JSONL index 已跑通；EXIF/GPS、标签、pHash、local visual embedding 和粗粒度照片搜索已通过；细粒度服装/属性搜索仍是下一步 |
...
| ID | 标题 | 状态 | DoD |
| --- | --- | --- | --- |
| B-011 | 真实 Personal root 集成 | verified | 受控真实测试文件、索引刷新、搜索、下载链路通过 gate；每次换真实部署仍需重新确认挂载和 ACL |
