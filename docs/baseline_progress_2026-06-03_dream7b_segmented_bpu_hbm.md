...
- treat selected pair `[1, 8]` as a positive telemetry-backed warm-path optimization; the guarded candidate service now passes 16-request service validation, but `default_service_replaced: False`, so the next change must compare candidate-vs-default service telemetry before any default replacement;
