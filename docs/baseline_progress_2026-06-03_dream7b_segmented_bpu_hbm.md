...
- use the verified `dream7b-bpu-resplit-batch-forward` telemetry as the optimization baseline before any default-service promotion; batch 16 reaches logits and spikes to `max_bpu_loading: 100.0`, but `avg_bpu_loading: 8.697` and `load_to_run_ratio: 9.817642` still show the route is load/residency dominated;
