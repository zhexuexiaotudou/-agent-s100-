#!/usr/bin/env python3
"""Dump Dream7B/S100P segment boundaries with configurable saved segments.

Run on the S100P research host. This is an offline evidence tool; it loads HBM
segments one at a time and never touches product routes or services.
"""
...
def parse_segments(spec: str) -> set[int]:
    out: set[int] = set()
    for part in spec.split(","):
        ...
        if ":" in part or "-" in part:
            sep = ":" if ":" in part else "-"
            a, b = [int(x) for x in part.split(sep, 1)]
...
def run_single_case(args: argparse.Namespace) -> int:
    ...
    save_segments = parse_segments(args.save_segments)
...
def parser_base() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--save-segments", default="20:27")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--single-case-json" in argv:
        args = parser_base().parse_args(argv)
    ...
    for case in cases:
        ...
        for seg in result.get("segments", []):
            if seg.get("segment") in parse_segments(args.save_segments) and (seg.get("dequant_stats") or {}).get("constant"):
    ...
    payload = {
        "schema_version": "dream7b_s100p_boundary_dump_v6",
        "created_at_utc": utc_now_iso(),
        "run_id": root.name,
        "s100p_boundary_dump_verdict": verdict,
        "save_segments": sorted(parse_segments(args.save_segments)),
    ...
    lines = [
        "# S100P Boundary Dump v6",
        "",
        f"- verdict: `{verdict}`",
        f"- save_segments: `{args.save_segments}`",
