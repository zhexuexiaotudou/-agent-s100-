...
APPLY=0
SOURCE_ROOT=""
TARGET="/mnt/nas/openclaw/scripts/probes/ai_nas_identity.py"
BACKUP_BASE="/mnt/nas/openclaw/reports/product_delivery/identity_runtime_backups"
...
SOURCE="$SOURCE_ROOT/scripts/probes/ai_nas_identity.py"
if [[ -n "$ROLLBACK_FROM" ]]; then SOURCE="$ROLLBACK_FROM/ai_nas_identity.py"; fi
...
backup_file="$backup_root/ai_nas_identity.py"
mkdir -p "$backup_root"
...
if ! systemctl --user restart "$SERVICE"; then
  restore_previous
  printf 'user service restart failed; restored previous identity runtime\n' >&2
  exit 1
fi

healthy=0
for _ in $(seq 1 20); do
  if curl -fsS --max-time 3 "$PORTAL_HEALTH" >/dev/null; then healthy=1; break; fi
  sleep 1
done
if [[ "$healthy" != "1" ]]; then
  restore_previous
  printf 'portal health check failed; restored previous identity runtime\n' >&2
  exit 1
fi
...
if [[ "$deployed_sha" != "$source_sha" ]]; then
  restore_previous
  printf 'identity runtime hash mismatch; restored previous runtime\n' >&2
  exit 1
fi
