...
\begin{titlepage}
  ...
  \vspace{1.0em}
  \answerbox{
  当前已经从本地 workspace fallback 推进到 NAS-backed smoke baseline：S100P 常驻 OpenClaw Gateway，TS-264C 通过 NFS v4.1 持久化挂载到 \texttt{/mnt/nas/openclaw}，文档索引、每日摘要、日志诊断、浏览器截图、ROS bag session、300 秒命名采集、dataset card、实验报告、稳定性采样和安全审计均可写入 NAS。
  }
  ...
  \begin{itemize}
    ...
    \item 未完成项集中在长期稳定性、sandbox runtime、服务收敛执行、Home Assistant 真实配置、控制策略和真实业务数据填充。
  \end{itemize}
...
ROS bag 采集 & verified & NAS-backed start/status/stop self-test 通过；命名采集 policy 已生成；一次人工批准的 300 秒 named capture 已完成，\texttt{record\_exit=0}、\texttt{metadata\_exists=yes}。 \\
稳定性采样 & doing & systemd timer 与 overnight runner 均已写 NAS；当前 14 个 snapshot、5.63h，仍需 168 小时样本。 \\
工具白名单 & verified & narrow plugin/runner 可用；2026-05-28 负向复测中 agent 拒绝非白名单 \texttt{/usr/bin/touch}，marker 未创建。 \\
...
图片 caption / 搜索 & 图片 metadata caption 与 JSONL index；语义 caption readiness 已确认缺本地视觉模型 & doing \\
日志诊断 & 从 NAS logs 输出错误摘要、关键匹配和建议命令 & verified \\
机器人数据集管理 & ROS bag session 和 300 秒 named capture 写 NAS，并生成 \texttt{DATASET\_CARD.md} & verified \\
...
项目 & 路径 \\
\midrule
Baseline roll-up & \path{/mnt/nas/openclaw/reports/baseline-status/baseline_status_20260528-235400.md} \\
Stability summary & \path{/mnt/nas/openclaw/reports/stability/stability_summary_20260528-235359.md} \\
...
ROS bag named capture & \path{/mnt/nas/openclaw/logs/probes/rosbag_named_capture_20260528-231319.md} \\
Dataset card & \path{/mnt/nas/openclaw/robot_datasets/approved_named_capture_20260528-231319/DATASET_CARD.md} \\
ROS bag capture policy & \path{/mnt/nas/openclaw/logs/probes/rosbag_capture_policy_20260528-224523.md} \\
...
Vision caption readiness & \path{/mnt/nas/openclaw/reports/image-captions/vision_caption_readiness_20260528-230810.md} \\
Security audit & \path{/mnt/nas/openclaw/logs/probes/security_audit_20260528-232340.md} \\
Service policy & \path{/mnt/nas/openclaw/logs/probes/service_policy_20260528-232101.md} \\
Service convergence decision & \path{/mnt/nas/openclaw/reports/security/service_convergence_decision_20260528-235327.md} \\
Control action policy & \path{/mnt/nas/openclaw/logs/probes/control_action_policy_20260528-225702.md} \\
Overnight runner & \path{/mnt/nas/openclaw/logs/overnight/overnight_baseline_20260528-232330.jsonl} \\
\bottomrule
\end{longtable}
\normalsize
...
事项 & 当前阻塞 & 下一步 \\
\midrule
A-010 稳定性 & 需要 168 小时样本 & timer 与 overnight runner 已在 NAS 采样，等待时间累计后生成验收 summary。 \\
A-006 Sandbox & 无 Docker/Podman/runc & 决定安装 runtime 或从第一版 baseline drop。 \\
B-003 图片 caption & metadata caption 已验证；语义 readiness 结论为缺本地语义模型 & 决定是否保持 metadata-only，或安装/挂载本地语义视觉模型。 \\
B-008 设备状态 & 缺 Home Assistant URL/token & 用户提供配置后仅调用 \texttt{GET /api/} 和 \texttt{GET /api/states}。 \\
B-009 控制动作 & disabled policy 已通过预检 & 补真实 reviewed action、二次确认文案和 request/approve/execute 审计。 \\
B-010 服务收敛 & decision pack 已给出建议；需确认 NFS/RPC、x11vnc、iiod 是否可关 & 只在确认后执行 disable/firewall，并在执行后重跑 security audit。 \\
\bottomrule
\end{longtable}
...
下一阶段不应继续堆 smoke demo，而应把真实文档、真实图片、真实机器人数据和 7 天稳定性样本接入同一套 NAS-backed 报告链路，并在服务收敛执行和控制策略确认后再进入更接近产品化的验收。
