...
class ProductAccessHttpTest(unittest.TestCase):
    ...
    def test_product_access_serves_packaged_ui_instead_of_stale_upstream_assets(self):
        ...
        self.assertIn("20260718-live-media", page)
        content_security_policy = response.getheader("Content-Security-Policy")
        self.assertIn("img-src 'self' data: blob:", content_security_policy)
        self.assertIn("media-src 'self' blob:", content_security_policy)
        connection.close()

        connection = http.client.HTTPConnection("127.0.0.1", self.server.server_port)
        connection.request("GET", "/static/digua_ai_nas_v2.js?v=20260718-live-media")
    ...
    def test_pwa_service_worker_excludes_api_and_downloads(self):
        ...
        self.assertEqual(response.status, 200)
        self.assertIn("digua-shell-v3", script)
