...
- [scripts/probes/dream7b_bpu_batch_queue_systemd_drain_probe.sh](scripts/probes/dream7b_bpu_batch_queue_systemd_drain_probe.sh): submits a multi-request JSONL job through the NAS-backed systemd queue and verifies default `--drain-all` processing, including the recorded five-request `[4, 1]` and eight-request `[4, 4]` batch splits.
