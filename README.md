...
- [scripts/dream7b-bpu-text-queue-run.sh](scripts/dream7b-bpu-text-queue-run.sh): reusable S100P command that calls `dream7b-bpu-text-queue-submit`, waits for `dream7b-bpu-batch-queue.service`, and returns BPU `topk_last_position` output.
- [scripts/probes/dream7b_bpu_text_queue_systemd_probe.sh](scripts/probes/dream7b_bpu_text_queue_systemd_probe.sh): verifies the text queue systemd path by calling `dream7b-bpu-text-queue-run` and checking service status, queue summary, tokenizer, and BPU output fields.
...
- [scripts/probes/dream7b_bpu_deployment_acceptance_probe.sh](scripts/probes/dream7b_bpu_deployment_acceptance_probe.sh): report-only deployment acceptance gate that aggregates the latest Dream 7B BPU service, HBM artifact inventory, batch capacity, systemd batch/drain/canary/text-queue/telemetry, long-repeat, and queue-retention reports.
