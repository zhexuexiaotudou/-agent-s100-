...
- Current S100 official LLM/Qwen baseline: `/mnt/nas/openclaw/reports/models/s100_official_llm_baseline_20260606-000204/official_llm_baseline_probe.json` reports `verdict: ok_s100_official_llm_baseline_probe`, `sdk_exists: True`, `config_dir_count: 8`, `official_hbm_download_entry_count: 14`, `qwen_existing_hbm_count: 0`, `similar_issue_evidence_available_for_official_qwen: False`, and `next_probe_target: download one official Qwen .hbm listed in resolve_model_nash-m.txt and run the matching oellm runtime example with hrt_ucp_monitor before using it as a utilization baseline`.
