...
- [scripts/probes/dream7b_bpu_text_queue_systemd_probe.sh](scripts/probes/dream7b_bpu_text_queue_systemd_probe.sh): encodes a real Dream 7B prompt through the S100P tokenizer, submits the tokenized request through the NAS-backed systemd queue, and verifies BPU `topk_last_position` output.
