...
- [scripts/probes/dream7b_bpu_batch_queue_systemd_drain_probe.sh](scripts/probes/dream7b_bpu_batch_queue_systemd_drain_probe.sh): submits a five-request JSONL job through the NAS-backed systemd queue and verifies default `--drain-all` processing with `batch_counts` `[4, 1]`.
