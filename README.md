...
- [scripts/probes/dream7b_bpu_batch_queue_systemd_drain_probe.sh](scripts/probes/dream7b_bpu_batch_queue_systemd_drain_probe.sh): submits a multi-request JSONL job through the NAS-backed systemd queue and verifies default `--drain-all` processing. Historical reports include five-request `[4, 1]` and eight-request `[4, 4]` splits at `max_batch_size=4`; the current service default is `--max-batch-size 8` and the latest eight-request drain report verifies `[8]`.
